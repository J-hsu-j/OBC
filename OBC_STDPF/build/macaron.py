...
# The SDK code generators (gen_bp_instances.py, hsmgen, ...) open UTF-8 FIDL/C files
# without specifying an encoding. On Windows with a non-UTF-8 system locale (e.g. cp950)
# this raises UnicodeDecodeError. Forcing UTF-8 mode here propagates to every child
# process (cmake -> ninja -> python), so the build works regardless of the caller's env.
...
class config:
    ...
    def get_java_exe() -> str:
        ...
        if not os.getenv(config.ESPF_JAVA_EXE_ENV_VAR):
            if shutil.which(java_file_name) is None:
                # No java on PATH: fall back to the JRE bundled with STM32CubeIDE, if installed.
                bundled = config.find_stm32cubeide_java(java_file_name)
                if bundled is not None:
                    log.warning(
                        f'{config.ESPF_JAVA_EXE_ENV_VAR} not set and `{java_file_name}` not on PATH. Using STM32CubeIDE bundled JRE: "{bundled}"'
                    )
                    # Export so nested macaron invocations (spawned by ninja) reuse it.
                    os.environ[config.ESPF_JAVA_EXE_ENV_VAR] = bundled
                    return bundled
    ...
    @staticmethod
    def find_stm32cubeide_java(java_file_name: str):
        """Locate java bundled with an STM32CubeIDE installation (Windows only). Returns a path string or None."""
        if os.name != 'nt':
            return None

        candidates = []
        for drive in ('C:', 'D:'):
            for base in (Path(f'{drive}/ST'), Path(f'{drive}/Program Files/STMicroelectronics')):
                if base.is_dir():
                    candidates.extend(base.glob(f'STM32CubeIDE*/STM32CubeIDE/plugins/com.st.stm32cube.ide.jre.*/jre/bin/{java_file_name}'))

        # Prefer the newest IDE version if several are installed
        candidates = sorted((c for c in candidates if c.is_file()), key=lambda p: str(p), reverse=True)
...
class commands:
    ...
    @staticmethod
    def run_build(configuration: str, cmake_pass_through_args: str) -> None:
        ...
        cmake_gen = ['cmake',
                     ...
                     f'--graphviz={configuration}/dot/OBC.dot']
        cmake_build = ['cmake', '--build', '.', '--config', configuration]

        cmake_gen.extend(cmake_pass_through_args)
    ...
    class build_version_action(argparse.Action):
        ...
        def __call__(self, parser, namespace, values, option_string=None):
            if re.search(config_file_updater.build_version_action.BUILD_VERSION_REGEX, values):
                ...
                raise ValueError(
                    'Release version shall use the following format: \n\t<1/2 digits>.<1/2 digits>.<1/2 digits>[-<alphanumerical text (\'_\', \'-\' and \'.\' characters are also allowed)>]'
                )
...
if __name__ == '__main__':
    ...
    parser.add_argument('-pt', '--pass-through', type=str,
                               default="", help='the options are directly passed to macchiato')
    parser.add_argument('-ptcmake', '--pass-through-cmake', type=str,
                               default="", nargs=argparse.REMAINDER, help='the options are directly passed to cmake')
    ...
    try:
        ...
        # FP code generation
        if args.fp_gen or args.datacache_gen or args.pass_through:
            if args.depl_config:
                ...
                if args.fp_gen and args.fp_merge_root:
                    ...
                    if args.fp_gen:
                        ...
                        commands.run_macchiato(args.pass_through, depl_config)
                        macaron_utils.clean_directories_by_name(
                            args.clean_gen_content.resolve(), 'src-gen-upy')
            else:
                raise Exception(
        ...
        if args.build:
            ...
            commands.run_build(args.build, args.pass_through_cmake)
