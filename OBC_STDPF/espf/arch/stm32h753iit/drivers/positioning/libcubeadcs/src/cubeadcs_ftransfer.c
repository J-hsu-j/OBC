...
static const char *pccFTypes[FILE_TYPE_MAX] = {
    "Tlm",
    "Jpg",
    "Bmp",
    "Ind",
...
static eCubeADCS_StdRply fs_CubeADCS_FTransfer_CalcSavedBlockCRC16(uint16_t *pu16Cache, fm_handle_t psFDwnObj, char *pcFName, uint32_t u32Len);
...
void CubeADCS_FTransfer_TaskInit(void)
{
    ...
    task_mon_register_for_stats(TASK_ID_CUBEADCS_FHANDL, CubeADCS_FTransf_TaskHandle);

    task_mon_task_initialized(TASK_ID_CUBEADCS_FHANDL);
...
static void fs_CubeADCS_FTransfer_FHandlTask(void *arg)
{
    for (;;)
    {
        ...
        task_mon_i_am_alive(TASK_ID_CUBEADCS_FHANDL);
        task_mon_low_stack_check(TASK_ID_CUBEADCS_FHANDL);
    }
}
...
static void fs_CubeADCS_FTransfer_DownloadFile_Init()
{
    ...
    sFDwn_P.u32FPktCnt          = (sFTransfMsg.sFDwn->FDwnInfo.u32FileSize % CUBEADCS_FDWN_BURST_PACKET_SIZE) == 0 ? (sFTransfMsg.sFDwn->FDwnInfo.u32FileSize / CUBEADCS_FDWN_BURST_PACKET_SIZE) : (sFTransfMsg.sFDwn->FDwnInfo.u32FileSize / CUBEADCS_FDWN_BURST_PACKET_SIZE) + 1;
    ...
    {
        if (FM_RES_OK == fm_open(&sFDwnObj, &sFDwn_P.cFName[0], FM_MODE_WRITE | FM_MODE_CREATE_ALWAYS))
        {
            (void)fm_close(&sFDwnObj);
...
static void fs_CubeADCS_FTransfer_DownloadFile_StartDwnBurst(void)
{
    ...
    if ((++burstCnt > CUBEADCS_FTRANSG_FDWN_BURST_MAX_RETRY) || (eCUBEADCS_STDRPLY_OK != fs_CubeADCS_FTransfer_SendCmdWithRpt(&sCmd_DwnBurstCmd, eCUBEADCS_COMMREQTYPE_TX_ONLY, CUBEADCS_FTRANSF_FDWN_RPT)) || (FM_RES_OK != fm_open(&sFDwnObj, &sFDwn_P.cFName[0], FM_MODE_WRITE | FM_MODE_OPEN_APPEND)))
    {
...
static void fs_CubeADCS_FTransfer_DownloadFile_PollDwnBurst(void)
{
    ...
    switch (retStatus)
    {
        ...
        case eCUBEADCS_COMM_RX_TIMEOUT:
            ...
            for (uint8_t pkt = 0; pkt < (p_cubeadcs_proc_queue->tail / CUBEADCS_TLM255_LEN); pkt++)
            {
                ...
                if ((u16Pkt >= CUBEADCS_FDWN_MAX_PKT_CNT) || (FM_RES_OK != fm_seek(&sFDwnObj, (sFDwn_P.u32FileSize + ((uint32_t)u16Pkt * 0x14)))) || ((FM_RES_OK != fm_write(&sFDwnObj, (p_base_address + 3), 0x14, &u32WrtCnt)) || (u32WrtCnt != 0x14)))
                {
                    fs_CubeADCS_FTransfer_UpdateFDwnFPtr(eCUBEADCS_FTRANSF_FDWNSTATE_START_DWN_BURST);
    ...
    else
    {
        ...
        if (FM_RES_OK != fm_close(&sFDwnObj))
        {
            fs_CubeADCS_FTransfer_UpdateFDwnFPtr(eCUBEADCS_FTRANSF_FDWNSTATE_ERR);
        }
    }
            ...
            if (eCUBEADCS_STDRPLY_OK == fs_CubeADCS_FTransfer_CalcSavedBlockCRC16(&u16CalcCRC, &sFDwnObj, &sFDwn_P.cFName[0], sCmdRsp.TLM242.BlockLen))
            {
                if (u16CalcCRC == sCmdRsp.TLM242.Crc16)
                {
                    ...
                    else
                    {
                        ...
                        if (FM_RES_OK == fm_open(&sFDwnObj, &sFDwn_P.cFName[0], FM_MODE_WRITE | FM_MODE_READ))
                        {
                            fm_truncate(&sFDwnObj, sFTransfMsg.sFDwn->FDwnInfo.u32FileSize);
                        }
                        (void)fm_close(&sFDwnObj);
                    }
                }
...
static void fs_CubeADCS_FTransfer_DownloadFile_Done(void)
{
    ...
    if (FM_RES_OK == fm_open(&sFDwnObj, &sFDwn_P.cFName[0], FM_MODE_READ))
    {
        (void)fm_close(&sFDwnObj);

        for (uint8_t u8RtrCnt = 0; u8RtrCnt <= CUBEADCS_FTRANSF_FDWN_RPT; ++u8RtrCnt)
        {
            uint32_t sFDwnObj_size;
            fm_size(&sFDwnObj, &sFDwnObj_size);
            if (eCUBEADCS_STDRPLY_OK == fs_CubeADCS_FTransfer_CalcSavedBlockCRC16(&u16CalcCRC, &sFDwnObj, &sFDwn_P.cFName[0], sFDwnObj_size))
            {
...
static void fs_CubeADCS_FTransfer_DownloadFileList_Init(void)
{
    ...
    if (true == fm_storage_status_ready(FM_VOL_SD_PATH))
    {
        if (FM_RES_OK == fm_open(&sFListObj, &ccFListName[0], FM_MODE_WRITE | FM_MODE_CREATE_ALWAYS))
        {
            fm_printf(&sFListObj, NULL, "CubeADCS File List\r\n--------------------------------------------\r\n");
            fm_printf(&sFListObj, NULL, "FileType (s|d)\t Size\t CRC\t Time\t Ctr\r\n\r\n");

            if (FM_RES_OK == fm_close(&sFListObj))
            {
    ...
    if (eCUBEADCS_STDRPLY_OK == fs_CubeADCS_FTransfer_SendCmdWithRpt(&sCmd_GetFileInformation, eCUBEADCS_COMMREQTYPE_FETCH, CUBEADCS_FTRANSF_FDWN_RPT))
    {
        ...
        else
        {
            ...
            else
            {
                ...
                if (FM_RES_OK == fm_open(&sFListObj, &ccFListName[0], FM_MODE_WRITE | FM_MODE_OPEN_APPEND))
                {
                    uint32_t size;
                    if ((FM_RES_OK == fm_size(&sFListObj, &size)) && (FM_RES_OK == fm_seek(&sFListObj, size)))
                    {
                        const char *pcFType;
                        ...
                        fm_printf(&sFListObj, NULL, "%s | %d\t 0x%08lX\t 0x%04X\t %02d/%02d/%04d %02d:%02d:%02d\t %03d\r\n",
                                  pcFType,
                                  sCmdRsp.TLM243.Byte0.Data.fileType,
                                  sCmdRsp.TLM243.Size,
...
static eCubeADCS_StdRply fs_CubeADCS_FTransfer_CalcSavedBlockCRC16(uint16_t *pu16Cache, fm_handle_t psFDwnObj, char *pcFName, uint32_t u32Len)
{
    ...
    uint32_t fBurstBaseByteCnt;

    if (FM_RES_OK == fm_open(psFDwnObj, pcFName, FM_MODE_READ))
    {
        eRetStatus = eCUBEADCS_STDRPLY_OK;

        uint32_t sFDwnObj_size;
        (void)fm_size(psFDwnObj, &sFDwnObj_size);

        (void)fm_close(psFDwnObj);

        if (sFDwnObj_size != u32Len)
        {
            fBurstBaseByteCnt = sFDwnObj_size - ((u32Len % 20 == 0) ? u32Len : (u32Len + (20 - u32Len % 20)));
        }
        else
        {
            fBurstBaseByteCnt = 0;
        ...
        eFRes = fm_open(psFDwnObj, pcFName, FM_MODE_OPEN_EXISTING | FM_MODE_READ);
        if (FM_RES_OK == eFRes)
        {
            for (uint32_t u32Indx = 0; u32Indx < u32Len; ++u32Indx)
            {
                eFRes = fm_seek(psFDwnObj, fBurstBaseByteCnt + u32Indx);
                if (FM_RES_OK == eFRes)
                {
                    (void)fm_read(psFDwnObj, (void *)&u8ReadData, 1, &uintReadBytes);
