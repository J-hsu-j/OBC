...
mod_op_status_id_t cube_adcs_init(sys_init_level_id_t level)
{
    ...
    {
        fs_cubeadcs_init_os();
        task_mon_set_task_policy(TASK_ID_CUBEADCS, TASK_MON_POLICY_MONITOR);
        task_mon_set_task_policy(TASK_ID_CUBEADCS_FHANDL, TASK_MON_POLICY_MONITOR);
        ret = STATUS_OK;
    }
    else
    {
...
mod_op_status_id_t cube_adcs_deinit(sys_init_level_id_t level)
{
    ...
    switch (level)
    {
        ...
        case INIT_OS:
            ...
            task_mon_set_task_policy(TASK_ID_CUBEADCS, TASK_MON_POLICY_DONT_CARE);
            task_mon_set_task_policy(TASK_ID_CUBEADCS_FHANDL, TASK_MON_POLICY_DONT_CARE);
...
void CubeADCS_TaskInit()
{
    ...
    task_mon_register_for_stats(TASK_ID_CUBEADCS, CubeADCS_Task_TaskHandle);

    task_mon_task_initialized(TASK_ID_CUBEADCS);
}
...
eCubeADCS_StdRply cubeadcs_set_cms_status(const eCubeADCS_Commissioning_t newStatus)
{
    ...
    if (newStatus < eCUBEADCS_MAX_CNT)
    {
...
eCubeADCS_Comm_Status_t CubeADCS_SendCmd(CubeADCS_CmdCfg_t *sCmd, eCubeADCS_CommReqType_t eCommReqType)
{
    ...
    DATA_CACHE_FDIR_CUBEADCS_CMD_EXEC_STATUS_t exec_status;
    ...
    if (true == fs_cubeadcs_lock_comm_access(CUBEADCS_COMM_RX_TIMEOUT))
    {
        ...
        exec_status.u8Error_code = (uint8_t)(eRetStatus);
        dc_set_fdir_cubeadcs_cmd_exec_status_data(&exec_status);
        (void)dc_get_fdir_cubeadcs_cmd_exec_status_data(&exec_status);
        if (eCUBEADCS_COMM_OK != eRetStatus)
        {
            fdir_srv_log_if_clear(FDIR_FAULT_ADCS_CMD_EXEC_FAILURE, FDIR_AGENT_CUBE_ADCS, "CubeADCS Gen1 command failed with err code %d", eRetStatus);
        }
        (void)fdir_srv_fault_set_state(FDIR_FAULT_ADCS_CMD_EXEC_FAILURE, FDIR_AGENT_CUBE_ADCS, (0 != exec_status.u8Error_code), NULL);
...
eCubeADCS_StdRply CubeADCS_GetAvrgAngVel(CubeADCS_AngRateFormatted_t *sAvrgAngVel, eCubeADCS_AngRateType_t eMeasType, uint8_t u8SCntToDecision)
{
    ...
    else if (u8SCnt > u8SCntToDecision)
    {
        EXEH_vException(eEXEHSeverity_Error, eEXEHModuleID_CUBEADCS, eEXEH_CUBEADCS_EXCEPTION_RUNTIME_ERROR, __LINE__);
    }
    else
...
static void fs_cubeadcs_task(void *arg)
{
    for (;;)
    {
        ...
        task_mon_i_am_alive(TASK_ID_CUBEADCS);
        task_mon_low_stack_check(TASK_ID_CUBEADCS);
    }
}
        ...
        case eCUBEADCS_CMD_ID_224: /* TLM224 */
            {
                if (eCUBEADCS_MODE_APPLICATION == eMode)
                {
                    ...
                    u32ErrState = (((uint32_t)sStateADCS2.Byte2.u8DataByte << 16) +
                                   ...
                                   ((((uint32_t)sStateADCS2.Byte0.u8DataByte << 2) & 0x3C) | (sStateADCS2.Byte0.u8DataByte & 0xC0))) >>
                                  2;
...
static uint8_t fs_cubeadcs_attitude_change_mode_check(void)
{
    ...
    if (eCUBEADCS_CFGTYPE_ATTITUDE_CHANGE == sCurrentSysStateCfg.eReqCfg)
    {
        ...
        if (true == conops_get_thresh_val(&i32OmegaLowThresh, CONOPS_THRESHOLDVALUETYPES_ANGVEL_Y_THOMSON_LOW))
        {
            if (true == conops_get_thresh_val(&i32OmegaHighThresh, CONOPS_THRESHOLDVALUETYPES_ANGVEL_Y_THOMSON_HIGH))
            {
