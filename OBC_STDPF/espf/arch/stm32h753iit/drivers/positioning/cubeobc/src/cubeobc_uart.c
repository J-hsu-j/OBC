...
bool cubeobc_uart_init(void)
{
    ...
    if (NULL == fs_thread)
    {
        ...
        task_mon_register_for_stats(TASK_ID_CUBEADCS_GEN2_COMM, fs_thread);
    }
...
static void fs_uart_thread(void *args)
{
    ...
    task_mon_task_initialized(TASK_ID_CUBEADCS_GEN2_COMM);
    task_mon_set_task_policy(TASK_ID_CUBEADCS_GEN2_COMM, TASK_MON_POLICY_MONITOR);

    while (1)
    {
        task_mon_i_am_alive(TASK_ID_CUBEADCS_GEN2_COMM);
        ...
        task_mon_low_stack_check(TASK_ID_CUBEADCS_GEN2_COMM);
    }
}
