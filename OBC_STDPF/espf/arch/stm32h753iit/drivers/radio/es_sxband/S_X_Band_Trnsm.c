...
#define S_X_BAND_STACK_RETR_SUB(condition, stackEntry) \
    if ((condition) && ((stackEntry)->retries > 0U))   \
    (stackEntry)->retries--
#define DECR_IF_NONZERO(var) \
    if ((var) > 0U)          \
    (var)--
...
#define UNBLOCK_CACHE_THREAD_FLAG (0x02) /**< Event flag value used to unblock the cache update calling thread when the operation is finished */
...
#define STR_INVALID_DATA_PTR "Invalid data pointer"
#define STR_INVALID_DATA_LEN "Ivalid data length"
#define STR_STACK_TIMEOUT    "Stack timeout"
#define STR_TXBUF_LOW        "Transmit buffer is insufficient"
#define STR_UART_RESERVED    "Could not reserve USART interface"

static uint8_t               file_tmp_buf[FILE_TEMP_BUFFER_SIZE];
static fm_file_obj_t         fhnd;
static file_stream_fmt_ctx_t fstream_ctx = {
    .p_data_buf      = file_tmp_buf,
    .data_buf_size   = sizeof(file_tmp_buf),
    .fop_status_last = FM_RES_OK,
...
typedef struct X_BAND_CMD_StackEntry
{
    ...
    // this field contains the full path name of a file as it is on the OBC SD card (including folders);
    // it cannot be used with SXBand modules because they do not support folders on the file system (hence the name sent to the
    // module is only the file name part for file operations)
    char                          obc_file_name[FM_MAX_PATH];
...
typedef struct
{
    ...
    fm_file_obj_t fhnd;                     /**< File handle for the cached SXBand file data */
...
static const S_X_BAND_TRNSM_CMD_INFO_TxInfo_struct TxCMD_Descriptor[S_X_BAND_TRNSM_CMD_NUMBER] = {
 ...
 /* X_BAND_TRNSM_CMD_SET,          */
    { S_X_BAND_TRNSM_CMD_VAL_SET,           112,                            1                             },
 /* X_BAND_TRNSM_CMD_DIR_F,        */
    { S_X_BAND_TRNSM_CMD_VAL_DIR_F,         0,                              S_X_BAND_TRNSM_MAX_READ_BUFFER},
 /* X_BAND_TRNSM_CMD_DIR_NEXT_F,   */
    { S_X_BAND_TRNSM_CMD_VAL_DIR_NEXT_F,    0,                              S_X_BAND_TRNSM_MAX_READ_BUFFER},
 /* X_BAND_TRNSM_CMD_DELL_F,       */
    { S_X_BAND_TRNSM_CMD_VAL_DELL_F,        31,                             1                             },
 /* X_BAND_TRNSM_CMD_DELL_ALL_F,   */
    { S_X_BAND_TRNSM_CMD_VAL_DELL_ALL_F,    0,                              1                             },
 /* X_BAND_TRNSM_CMD_CREATE_F,     */
    { S_X_BAND_TRNSM_CMD_VAL_CREATE_F,      31,                             1                             },
 /* X_BAND_TRNSM_CMD_WRITE_F,      */
    { S_X_BAND_TRNSM_CMD_VAL_WRITE_F,       S_X_BAND_TRNSM_MAX_READ_BUFFER, 1                             },
 /* X_BAND_TRNSM_CMD_OPEN_F,       */
    { S_X_BAND_TRNSM_CMD_VAL_OPEN_F,        31,                             1                             },
 /* X_BAND_TRNSM_CMD_READ_F,       */
    { S_X_BAND_TRNSM_CMD_VAL_READ_F,        4,                              S_X_BAND_TRNSM_MAX_READ_BUFFER},
 /* X_BAND_TRNSM_CMD_SEND_F,       */
    { S_X_BAND_TRNSM_CMD_VAL_SEND_F,        31,                             1                             },
 /* X_BAND_TRNSM_CMD_TX_MODE,      */
    { S_X_BAND_TRNSM_CMD_VAL_TX_MODE,       0,                              1                             },
 /* X_BAND_TRNSM_CMD_LOAD_MODE,    */
    { S_X_BAND_TRNSM_CMD_VAL_IDLE_MODE,     0,                              1                             },
 /* X_BAND_TRNSM_CMD_UPDATE_FW,    */
    { S_X_BAND_TRNSM_CMD_VAL_UPDATE_FW,     31,                             1                             },
 /* X_BAND_TRNSM_CMD_SAFE_SHUTDOWN,*/
    { S_X_BAND_TRNSM_CMD_VAL_SAFE_SHUTDOWN, 0,                              1                             },
 /* X_BAND_TRNSM_CMD_GET_RES,      */
    { S_X_BAND_TRNSM_CMD_VAL_GET_RESULT,    2,                              S_X_BAND_TRNSM_MAX_READ_BUFFER}
};
...
/* Low level functions */
static S_X_BAND_TRNSM_Response_enum S_X_BAND_TRNSM_SendCMD(uint16_t Identifier, S_X_BAND_TRNSM_CMD_enum CMD, uint16_t CMD_Type, const uint8_t *TxData, uint16_t TxDataLength);
static S_X_BAND_TRNSM_Response_enum S_X_BAND_TRNSM_GetResult(uint16_t Identifier, S_X_BAND_TRNSM_CMD_enum CMD, uint16_t CMD_Type, uint8_t *RxData, uint16_t *RxDataLength);
...
static void                         fdir_sx_band_dc_failed_send_cmd(
                            const char                  *str,
                            size_t                       len,
                            S_X_BAND_TRNSM_Response_enum response,
                            uint16_t                     Identifier,
                            S_X_BAND_TRNSM_CMD_enum      CMD,
                            uint16_t                     CMD_Type,
                            uint16_t                     TxDataLength);

/* A function to check whether the radio module is an X-Band type */
static inline bool IS_XBAND(void)
{
    return S_X_BAND_TRNSM_DEVTYPE_XBAND == XSBandDevType;
}
...
mod_op_status_id_t sx_band_init(sys_init_level_id_t level)
{
    ...
    else if (level == INIT_HW)
    {
        ...
        const uint32_t hal_state = HAL_UART_GetState(p_sx_band_uart_hnd);
        if ((hal_state & HAL_UART_STATE_READY) != 0)
        {
    ...
    else if (level == INIT_OS)
    {
        ...
        task_mon_register_for_stats(TASK_ID_S_X_BAND, S_X_BAND_Task_TaskHandle);

        // Set the task monitor policy
        task_mon_set_task_policy(TASK_ID_S_X_BAND, TASK_MON_POLICY_MONITOR);
        ret = STATUS_OK;
    }
    else
    {
...
mod_op_status_id_t sx_band_deinit(sys_init_level_id_t level)
{
    ...
    {
        task_mon_set_task_policy(TASK_ID_S_X_BAND, TASK_MON_POLICY_DONT_CARE);
    }
...
static void fdir_sx_band_dc_failed_send_cmd(
    ...
    uint16_t                     TxDataLength)
{
    DATA_CACHE_FDIR_S_X_BAND_CMD_EXEC_STATUS_t exec_status;
    bool                                       error_occured;
    exec_status.u16Error_code = response;
    fdir_srv_log_if_clear(FDIR_FAULT_S_X_BAND_CMD_EXEC_FAILURE, FDIR_AGENT_S_X_BAND, "%.*s. S/X-Band command failed with err code %d, id: %u, cmd: %d, cmd_type: %u, data_len: %u",
                          len,
                          str,
                          TxCMD_Descriptor[CMD].tx_data_max_size,
                          response,
                          Identifier,
                          CMD,
                          CMD_Type,
                          TxDataLength);
    dc_set_fdir_s_x_band_cmd_exec_status_data(&exec_status);
    (void)dc_get_fdir_s_x_band_cmd_exec_status_data(&exec_status);
    error_occured = (S_X_BAND_TRNSM_STAT_ACK != response);
    (void)fdir_srv_fault_set_state(FDIR_FAULT_S_X_BAND_CMD_EXEC_FAILURE, FDIR_AGENT_S_X_BAND, error_occured, NULL);
}
 ...
 * @param[input]      TxDataLength - Number of bytes to be transmitted
 *
...
static S_X_BAND_TRNSM_Response_enum S_X_BAND_TRNSM_SendCMD(uint16_t Identifier, S_X_BAND_TRNSM_CMD_enum CMD, uint16_t CMD_Type, const uint8_t *TxData, uint16_t TxDataLength)
{
    ...
    if (CMD >= S_X_BAND_TRNSM_CMD_GET_RES)
    {
        ...
        fdir_sx_band_dc_failed_send_cmd(STR_INVALID_DATA_PTR, sizeof(STR_INVALID_DATA_PTR), S_X_BAND_TRNSM_STAT_WRONG_PARAM, Identifier, CMD, CMD_Type, TxDataLength);
        return S_X_BAND_TRNSM_STAT_WRONG_PARAM;
    }
    if ((TxDataLength + sizeof(S_X_BAND_TRNSM_Pack_struct) + sizeof(RxCRC)) > S_X_BAND_TRNSM_TX_BUFF_SIZE)
    {
        /* if CPU stops here - please increase the size of the buffer -> X_BAND_TRNSM_TX_BUFF_SIZE */
        len = (size_t)snprintf(err_buf, sizeof(err_buf), "Data length too long (above %u).", S_X_BAND_TRNSM_TX_BUFF_SIZE - sizeof(S_X_BAND_TRNSM_Pack_struct) + sizeof(RxCRC));
        len = (len < sizeof(err_buf)) ? len : sizeof(err_buf);
        fdir_sx_band_dc_failed_send_cmd(err_buf, len, S_X_BAND_TRNSM_STAT_WRONG_PARAM, Identifier, CMD, CMD_Type, TxDataLength);
        return S_X_BAND_TRNSM_STAT_WRONG_PARAM;
    }
    // Stack timeout
    if ((xTaskGetTickCount() - S_X_BAND_LastExecCmd_Tick) < (1 * S_X_BAND_STACK_DELAY))
    {
        fdir_sx_band_dc_failed_send_cmd(STR_STACK_TIMEOUT, sizeof(STR_STACK_TIMEOUT), S_X_BAND_TRNSM_STAT_GET_RESULT, Identifier, CMD, CMD_Type, TxDataLength);
    ...
    {
        if (TxCMD_Descriptor[CMD].tx_data_max_size >= TxDataLength)
        {
            TxPackStruct->Length = TxDataLength;
        }
        else
        {
            len = snprintf(err_buf, sizeof(err_buf), "Data unexpectedly long (above %u).", TxCMD_Descriptor[CMD].tx_data_max_size);
            len = (len < sizeof(err_buf)) ? len : sizeof(err_buf);
            fdir_sx_band_dc_failed_send_cmd(err_buf, len, S_X_BAND_TRNSM_STAT_WRONG_PARAM, Identifier, CMD, CMD_Type, TxDataLength);
            return S_X_BAND_TRNSM_STAT_WRONG_PARAM;
        }
    ...
    for (uint8_t retries = 0; retries < S_X_BAND_TRNSM_TX_RETRY;)
    {
        ...
        if (((((S_X_BAND_TRNSM_STRCT_SIZE + TxPackStruct->Length + sizeof(TxPackCRC) - 1) >> 4) + 1) << 4) > sizeof(TxDataBuffer))
        {
            ...
            fdir_sx_band_dc_failed_send_cmd(STR_TXBUF_LOW, sizeof(STR_TXBUF_LOW), S_X_BAND_TRNSM_STAT_WRONG_PARAM, Identifier, CMD, CMD_Type, TxDataLength);
            return S_X_BAND_TRNSM_STAT_WRONG_PARAM;
        }
        ...
        {
            S_X_BAND_LastExecCmd_Tick = xTaskGetTickCount();
            fdir_sx_band_dc_failed_send_cmd(STR_UART_RESERVED, sizeof(STR_UART_RESERVED), S_X_BAND_TRNSM_STAT_WRONG_PARAM, Identifier, CMD, CMD_Type, TxDataLength);
    ...
    DATA_CACHE_FDIR_S_X_BAND_CMD_EXEC_STATUS_t exec_status;
    exec_status.u16Error_code = RxPackStruct->Response;
    dc_set_fdir_s_x_band_cmd_exec_status_data(&exec_status);
    (void)dc_get_fdir_s_x_band_cmd_exec_status_data(&exec_status);
    if ((S_X_BAND_TRNSM_STAT_ACK != (S_X_BAND_TRNSM_Response_enum)exec_status.u16Error_code) &&
        (S_X_BAND_TRNSM_STAT_GET_RESULT != (S_X_BAND_TRNSM_Response_enum)RxPackStruct->Response))
    {
        fdir_sx_band_dc_failed_send_cmd("", 0, RxPackStruct->Response, Identifier, CMD, CMD_Type, TxDataLength);
    }
    else
    {
        (void)fdir_srv_fault_set_state(FDIR_FAULT_S_X_BAND_CMD_EXEC_FAILURE, FDIR_AGENT_S_X_BAND, false, NULL);
    }
 ...
 * @param[output]     RxDataLength - Number of bytes to be received
 *
 * @return            retStat - a value from enumeration S_X_BAND_TRNSM_Response_enum
 */
static S_X_BAND_TRNSM_Response_enum S_X_BAND_TRNSM_GetResult(uint16_t Identifier, S_X_BAND_TRNSM_CMD_enum CMD, uint16_t CMD_Type, uint8_t *RxData, uint16_t *RxDataLength)
{
    ...
    {
        /* Invalid data pointer */
        fdir_sx_band_dc_failed_send_cmd(STR_INVALID_DATA_LEN, sizeof(STR_INVALID_DATA_LEN), S_X_BAND_TRNSM_STAT_WRONG_PARAM, Identifier, CMD, CMD_Type, 0);
        return S_X_BAND_TRNSM_STAT_WRONG_PARAM;
    }
    else
    {
    ...
    {
        /* Invalid data pointer */
        fdir_sx_band_dc_failed_send_cmd(STR_INVALID_DATA_PTR, sizeof(STR_INVALID_DATA_PTR), S_X_BAND_TRNSM_STAT_WRONG_PARAM, Identifier, CMD, CMD_Type, *RxDataLength);
        return S_X_BAND_TRNSM_STAT_WRONG_PARAM;
    }
    TxPackStruct->Header   = S_X_BAND_TRNSM_HEADER;
    TxPackStruct->ModuleID = Identifier;
    ...
    for (uint8_t retries = 0; retries < S_X_BAND_TRNSM_TX_RETRY;)
    {
        ...
        if (((((S_X_BAND_TRNSM_STRCT_SIZE + TxPackStruct->Length + sizeof(TxPackCRC) - 1) >> 4) + 1) << 4) > sizeof(TxDataBuffer))
        {
            fdir_sx_band_dc_failed_send_cmd(STR_TXBUF_LOW, sizeof(STR_TXBUF_LOW), S_X_BAND_TRNSM_STAT_WRONG_PARAM, Identifier, CMD, CMD_Type, *RxDataLength);
            return S_X_BAND_TRNSM_STAT_WRONG_PARAM;
        }
        /* Reserve the USART interface for X-band */
        if (pdFALSE == S_X_Select(SX_SELECT_X_BAND_TRANSCEIVER))
        {
            fdir_sx_band_dc_failed_send_cmd(STR_UART_RESERVED, sizeof(STR_UART_RESERVED), S_X_BAND_TRNSM_STAT_WRONG_PARAM, Identifier, CMD, CMD_Type, *RxDataLength);
            return S_X_BAND_TRNSM_STAT_COMM_ERR;
        }
        ...
        if (S_X_Band_Index > 0)
        {
                ...
                (RxPackStruct->ModuleID == TxPackStruct->ModuleID))
            {
                ...
                ES_TRACE_DEBUG("RxCRC = 0x%08x RcvdCRC = 0x%08x\r\n", RxCRC, *pRxCRC);
        ...
        if (retries < S_X_BAND_TRNSM_TX_RETRY)
        {              // If maximum number of retries is not reached
            retries++; // Count one more retry
            if (retries == S_X_BAND_TRNSM_TX_RETRY)
            { // If last retry is done
                RxPackStruct->Response = S_X_BAND_TRNSM_STAT_NACK;
            }
            else
            {
    ...
    activate_fault = ((S_X_BAND_TRNSM_STAT_ACK != exec_status.u16Error_code) &&
                      (S_X_BAND_TRNSM_STAT_GET_RESULT != exec_status.u16Error_code));

    fdir_srv_log_if_not_same(FDIR_FAULT_S_X_BAND_CMD_EXEC_FAILURE, FDIR_AGENT_S_X_BAND, activate_fault, "S/X-Band command failed with err code %d, id: %u, cmd: %d, cmd_type: %u, data_len: %u",
                             TxCMD_Descriptor[CMD].tx_data_max_size,
                             RxPackStruct->Response,
                             Identifier,
                             CMD,
                             CMD_Type,
                             *RxDataLength);

    (void)fdir_srv_fault_set_state(FDIR_FAULT_S_X_BAND_CMD_EXEC_FAILURE, FDIR_AGENT_S_X_BAND, activate_fault, NULL);
...
static uint8_t S_X_BAND_TRNSM_CMD_SET_Param_Baudrate(S_X_BAND_CMD_StackEntry *stackEntry)
{
    ...
    switch (stackEntry->state)
    {
        ...
        case S_X_BAND_TRNSM_CMD_STATE_CMD_GETRESULT_RES:
            ...
            {
                // Verify the answer
                if (sizeof(retStat) == RxDataLength)
                {
                    // Update the state
                ...
                if (RxDataLength >= 4)
                {
                    ...
                    if (retStat == S_X_BAND_TRNSM_DIR_OK || retStat == S_X_BAND_TRNSM_DIR_NO_FILES)
                    {
                        ...
                        if (retStat == S_X_BAND_TRNSM_DIR_OK)
                        {
                            ...
                            memcpy(&stackEntry->ESTTC_data[sizeof(DirData->DirNextAvailable) + sizeof(DirData->NumberFiles)], &S_X_BAND_TRNSM_Result_Rx_Buffer[4], RxDataLength - 4);
...
static uint8_t S_X_BAND_TRNSM_DelFile(S_X_BAND_CMD_StackEntry *stackEntry)
{
    ...
    switch (stackEntry->state)
    {
        ...
        case S_X_BAND_TRNSM_CMD_STATE_CMD_RES:
            ...
            ES_TRACE_DEBUG("SXBand CMD res: %d\n", retStat);
        ...
        case S_X_BAND_TRNSM_CMD_STATE_CMD_GETRESULT_RES:
            ...
            ES_TRACE_DEBUG("Del file GetResult res: %d\n", retStat);
            ...
            if (S_X_BAND_TRNSM_STAT_GET_RESULT == retStat)
            {
                ...
                if (RxDataLength == sizeof(retStat))
                {
                    ...
                    {
                        ES_TRACE_DEBUG("Del file GetResult finished with res: %d\n", retStat);
...
static uint8_t S_X_BAND_TRNSM_CreateFile(S_X_BAND_CMD_StackEntry *stackEntry)
{
    ...
    switch (stackEntry->state)
    {
        ...
        case S_X_BAND_TRNSM_CMD_STATE_CMD_GETRESULT_RES:
            ...
            {
                // Verify the answer
                if (RxDataLength == sizeof(retStat) + sizeof(uint32_t))
                {
                    // Get the status of the request
                    retStat = S_X_BAND_TRNSM_Result_Rx_Buffer[0];
                    // If the status is "OK"
    ...
    switch (stackEntry->state)
    {
        ...
        case S_X_BAND_TRNSM_CMD_STATE_CMD_RES:
            ...
            if (S_X_BAND_TRNSM_STAT_ACK == retStat)
            {
                ...
                ES_TRACE_DEBUG("OpenFile ACK rcvd!\n");
            }
            else
            {
                // Failed to send the command
                ES_TRACE_DEBUG("=> OpenFile NACK!\n");
                stackEntry->state = S_X_BAND_TRNSM_CMD_STATE_CMD_RES;
                if ((S_X_BAND_TRNSM_STAT_COMM_ERR == retStat) && (!S_X_BAND_TRNSM_Scan_Baud_Rates(stackEntry->devID)))
                {
                    stackEntry->retries = 0;
        ...
        case S_X_BAND_TRNSM_CMD_STATE_CMD_GETRESULT_RES:
            ...
            ES_TRACE_DEBUG("OpenFile GetResult: %d\n", retStat);
            ...
            {
                // Verify the answer
                if (RxDataLength == sizeof(retStat) + sizeof(uint32_t) + sizeof(uint32_t))
                {
                    // Get the status of the request
                    retStat = S_X_BAND_TRNSM_Result_Rx_Buffer[0];
                    // If the status is "OK"
                    if ((retStat == S_X_BAND_TRNSM_OPEN_OK) || (retStat == S_X_BAND_TRNSM_OPEN_ERR))
                    {
                        ...
                        if (retStat == S_X_BAND_TRNSM_OPEN_OK)
                        {
                            ...
                            ES_TRACE_DEBUG("File info [hnd = 0x%04x, size = %ul\n");
                        }
                        // Update the state
                        stackEntry->state = S_X_BAND_TRNSM_CMD_STATE_CMD_FINISHED;
                        break;
...
static uint8_t S_X_BAND_TRNSM_SendFile(S_X_BAND_CMD_StackEntry *stackEntry)
{
    ...
    if (stackEntry->retries <= 0)
    {
        ...
        {
            retStat = S_X_BAND_TRNSM_PARAMS_COMM_ERR;
            ES_TRACE_DEBUG("sendfile failed\r\n");
        ...
        case S_X_BAND_TRNSM_CMD_STATE_CMD_GETRESULT_RES:
            ...
            if (S_X_BAND_TRNSM_STAT_GET_RESULT == retStat)
            {
                ...
                if (RxDataLength == sizeof(retStat))
                {
                    ...
                    ES_TRACE_DEBUG("sendfile: status = %d\r\n", retStat);

                    // If the status is "OK"
                    if (retStat == S_X_BAND_TRNSM_SEND_OK)
                    {
                        // Update the state
                        ES_TRACE_DEBUG("sendfile finished successfully (%lu ms)\r\n", timer_get_elapsed_time(&profile_timer));
                        timer_stop(&profile_timer);
                        stackEntry->state = S_X_BAND_TRNSM_CMD_STATE_CMD_FINISHED;
                        break;
                    }
...
static uint8_t S_X_BAND_TRNSM_DownloadFileFromSBandTrnsm(S_X_BAND_CMD_StackEntry *stackEntry)
{
    ...
    static fm_file_obj_t                             S_X_BAND_TRNSM_DownloadFromXSBand_file_descr;
    ...
    if (stackEntry->retries <= 0)
    {
        ES_TRACE_DEBUG("X_BAND_TRNSM_DownloadFileFromSBandTrnsm() FINISHED ERR, time =  %d\r\n", (int)xTaskGetTickCount());
        if (stackEntry->state != S_X_BAND_TRNSM_CMD_STATE_CMD_FINISHED)
        {
            retStat = S_X_BAND_TRNSM_PARAMS_COMM_ERR;
            ES_TRACE_DEBUG("ERROR\n");
        }
        return retStat;
    }
    // Start the complex command sequence
    if (stackEntry->expectSlaveCommand)
    {
        ES_TRACE_DEBUG("X_BAND_TRNSM_DownloadFileFromSBandTrnsm() START time =  %d\r\n", (int)xTaskGetTickCount());
        memcpy(&XSBandStack_Slave, stackEntry, sizeof(S_X_BAND_CMD_StackEntry));
        ...
        XSBandStack_Slave.parentEntry = stackEntry;
        ES_TRACE_DEBUG("Download START\n");
    }

    ES_TRACE_DEBUG("Download State = %d (retries: %d)\n", S_X_BAND_TRNSM_DownloadFromXSBand_State, stackEntry->retries);
    ...
    switch (S_X_BAND_TRNSM_DownloadFromXSBand_State)
    {
        case S_X_BAND_TRNSM_DownloadFromXSBand_State_OPENFILE:
            ...
            if ((!retStat) && (XSBandStack_Slave.state == S_X_BAND_TRNSM_CMD_STATE_CMD_FINISHED))
            {
                ...
                {
                    fm_result_t fr = fm_open(&S_X_BAND_TRNSM_DownloadFromXSBand_file_descr, (char *)stackEntry->ESTTC_data, FM_MODE_READ | FM_MODE_WRITE | FM_MODE_CREATE_ALWAYS);
                    retStat        = fr;
                    if (fr != FM_RES_OK)
                    {
                        // on disk error - try to reinitialise the SD card
                        if ((fr == FM_RES_DISK_ERR) || (fr == FM_RES_LOCKED))
                        {
                            ES_TRACE_DEBUG("X_BAND_TRNSM_DownloadFileFromSBandTrnsm() SD card. ERR = %d\r\n", fr);
                        }
                        // Close the file - regardless if the copy procedure is successful or not
                        fm_close(&S_X_BAND_TRNSM_DownloadFromXSBand_file_descr);
                    }
                    else
                    {
                        timer_start(&profile_timer);
        ...
        case S_X_BAND_TRNSM_DownloadFromXSBand_State_READFILE_GETRESULT:
            ...
            XSBandStack_Slave.timestamp = xTaskGetTickCount();
            // Send GetResult command
            retStat = S_X_BAND_TRNSM_GetResult(XSBandStack_Slave.devID, S_X_BAND_TRNSM_CMD_READ_F, S_X_BAND_TRNSM_NULL_TYPE, &FileDataBuffer.Status, &RxDataLength);
            if (S_X_BAND_TRNSM_STAT_GET_RESULT == retStat)
            {
                ...
                if (RxDataLength > 0)
                {
                    if (FileDataBuffer.PacketNumber == S_X_BAND_TRNSM_DownloadFromXSBand_PacketNumber)
                    {
                        ...
                        if (!retStat)
                        {
                            fm_result_t fr = fm_seek(&S_X_BAND_TRNSM_DownloadFromXSBand_file_descr, S_X_BAND_TRNSM_DownloadFromXSBand_FPos);
                            if (fr == FM_RES_OK)
                            {
                                uint32_t NumberReadBytes;

                                // Read a piece of file in the OBC
                                fr = fm_write(&S_X_BAND_TRNSM_DownloadFromXSBand_file_descr, &FileDataBuffer.Data, FileDataBuffer.Length, &NumberReadBytes);
                                if ((fr != FM_RES_OK) || (FileDataBuffer.Length != NumberReadBytes))
                                {
                                    // Wrong answer
                                    fm_close(&S_X_BAND_TRNSM_DownloadFromXSBand_file_descr);
                                    fr = fm_open(&S_X_BAND_TRNSM_DownloadFromXSBand_file_descr, (char *)stackEntry->ESTTC_data, FM_MODE_READ | FM_MODE_WRITE | FM_MODE_OPEN_EXISTING);
                                    if (fr != FM_RES_OK)
                                    {
                                        retStat = S_X_BAND_TRNSM_SEND_CARD_ERR;
                                        ES_TRACE_DEBUG("\r\n READ_F CMD fm_open(&) error = %d (retry No %d)", fr, stackEntry->retries);
                                    }
                                    else
                                    {
                                        retStat = 0x00;
    ...
    else if (XSBandStack_Slave.state == S_X_BAND_TRNSM_CMD_STATE_CMD_FINISHED)
    {
        ...
        {
            ES_TRACE_DEBUG("X_BAND_TRNSM_DownloadFileFromSBandTrnsm() FINISHED OK, time =  %d\r\n", (int)xTaskGetTickCount());
            timer_stop(&profile_timer);
            stackEntry->state = S_X_BAND_TRNSM_CMD_STATE_CMD_FINISHED;
            ES_TRACE_DEBUG("Download SUCCESS\n");
            return retStat;
        }
        XSBandStack_Slave.retries = stackEntry->retries;
    }
...
static uint8_t S_X_BAND_TRNSM_UploadFileToSBandTrnsm(S_X_BAND_CMD_StackEntry *stackEntry)
{
    ...
    if (stackEntry->retries <= 0)
    {
        ES_TRACE_DEBUG("S_X_BAND_TRNSM_UploadFileToSBandTrnsm() FINISHED tick =  %d\r\n", (int)xTaskGetTickCount());
        if (stackEntry->state != S_X_BAND_TRNSM_CMD_STATE_CMD_FINISHED)
        {
            retStat = S_X_BAND_TRNSM_PARAMS_COMM_ERR;
        }
    ...
    if (stackEntry->expectSlaveCommand)
    {
        ES_TRACE_DEBUG("X_BAND_TRNSM_UploadFileToSBandTrnsm() START tick =  %d\r\n", (int)xTaskGetTickCount());
        memcpy(&XSBandStack_Slave, stackEntry, sizeof(S_X_BAND_CMD_StackEntry));
        XSBandStack_Slave.expectSlaveCommand = stackEntry->expectSlaveCommand = 0;
    ...
    switch (S_X_BAND_TRNSM_UploadToXSBand_State)
    {
        case S_X_BAND_TRNSM_UploadToXSBand_State_OPEN_FILE:
            crypto_res = crypto_stream_open_always(&crypto_hnd, (char *)stackEntry->obc_file_name, OP_CMOX_AES_GCM_ENC);
            tx_len     = 0U;
            timer_start(&profile_timer);

            if (crypto_res != CRYPTO_API_RES_OK)
            {
                ES_TRACE_DEBUG("X_BAND_TRNSM_UploadFileToSBandTrnsm() Error crypto_stream_open_always(). ERR = %d\r\n", crypto_res);
        ...
        case S_X_BAND_TRNSM_UploadToXSBand_State_CREATEFILE:
            ...
            if ((0x00 == retStat) && (XSBandStack_Slave.state == S_X_BAND_TRNSM_CMD_STATE_CMD_FINISHED))
            {
                ...
                ES_TRACE_DEBUG("sxband-upload: SXBand file handle: %ld\r\n", S_X_BAND_TRNSM_FileDataBuffer.FileHandler);
            }
        ...
        case S_X_BAND_TRNSM_UploadToXSBand_State_WRITEFILE_GETRESULT:
            ...
            if (IS_TRUE(cmd_timer.is_running))
            {
                ES_TRACE_DEBUG("[WRITE-GETRES delta] => %ld\r\n", timer_get_elapsed_time(&cmd_timer));
                timer_stop(&cmd_timer);
            }

            ES_TRACE_DEBUG("GetResult ==> retStat = %d | op_res = %d | len = %ld\r\n", retStat, S_X_BAND_TRNSM_Result_Rx_Buffer[0], RxDataLength);
    ...
    else if (XSBandStack_Slave.state == S_X_BAND_TRNSM_CMD_STATE_CMD_FINISHED)
    {
        ...
        {
            ES_TRACE_DEBUG("S_X_BAND_TRNSM_UploadFileToSBandTrnsm() FINISHED OK, time =  %.2f min\r\n", (timer_get_elapsed_time(&profile_timer) / 1000.) / 60.);
...
void S_X_BAND_Task(void *argument)
{
    ...
    task_mon_task_initialized(TASK_ID_S_X_BAND);

    for (;;)
    {
        (void)osMutexAcquire(sxband_lock_id, osWaitForever);

        task_mon_i_am_alive(TASK_ID_S_X_BAND);
        ...
        task_mon_low_stack_check(TASK_ID_S_X_BAND);
    }
}
...
void S_X_BAND_TRNSM_ESTTC_StartCmd(uint16_t Identifier, S_X_BAND_TRNSM_GW_CMD_ID_enum Cmd, uint8_t Type, uint8_t size, const uint8_t *pData, sUserResponseCtx_t *pUserRespCtx, uint32_t *const p_cmd_eta_ms)
{
    ...
    if (accept_startcmd_req)
    {
             ...
             (S_X_BAND_TRNSM_GW_CMD_ID_SEND_FILES == Cmd)))
        {
            ...
            // wait for main task to execute all pending command on the stack before
            // going further with the cache command
            ...
            // make sure the thread ID is set to NULL to avoid undesired signaling from S_X_BAND_Task after
            // the cache has been updated (which may potentially unblock the caller of this function since it
            // takes place in the same thread context
        ...
        for (i = 0; i < S_X_BAND_STACK_SIZE; ++i)
        {
            if (XSBandStack[i].state == S_X_BAND_TRNSM_CMD_STATE_EMPTY)
            {
                ...
                XSBandStack[i].obc_file_name[0]   = '\0';
                XSBandStack[i].ESTTC_request_type = Type;

                if ((size > 0) && (NULL != pData))
                {
                    if (true == is_file_cmd)
                    {
                        // extract only file name for use by any file operations with SXBand SD card where folders are not supported
                        file_helper_parse_path((const char *)pData, size, (char *)NULL, 0, (char *)XSBandStack[i].ESTTC_data, (sizeof(XSBandStack[i].ESTTC_data) >= FM_MAX_PATH) ? FM_MAX_PATH : sizeof(XSBandStack[i].ESTTC_data));

                        XSBandStack[i].ESTTC_data_size                            = (uint8_t)strlen((char *)XSBandStack[i].ESTTC_data) + 1;
                        XSBandStack[i].ESTTC_data[XSBandStack[i].ESTTC_data_size] = '\0';

                        // copy the full path for use by local OBC operations with the file
                        //  (e.g. upload operations where you need to open the original file on the OBC SD card)
                        strncpy(XSBandStack[i].obc_file_name, (const char *)pData, (sizeof(XSBandStack[i].obc_file_name) >= size) ? size : sizeof(XSBandStack[i].obc_file_name));
                        XSBandStack[i].obc_file_name[size - 1] = '\0';
                    }
                    else
                    {
                        memcpy(XSBandStack[i].ESTTC_data, pData, size);
        ...
        {
            ES_TRACE_DEBUG("Error - Cannot add command to the stack !!!\r\n");
#ifdef S_X_BAND_STACK_DEBUG
            for (i = 0; i < S_X_BAND_STACK_SIZE; ++i)
            {
    ...
    switch (stackEntry->state)
    {
        ...
        case S_X_BAND_TRNSM_CMD_STATE_CMD_GETRESULT_RES:
            ...
            else
            {
                ...
                {
                    ES_TRACE_DEBUG("X_BAND_TRNSM_CMD_SET_Param GETRESULT error %d\r\n", (int)retStat);
                }
#endif
            }
            // Subtract the retry count
    ...
    switch (stackEntry->state)
    {
        ...
        case S_X_BAND_TRNSM_CMD_STATE_CMD_GETRESULT_RES:
            ...
            else
            {
                ...
                {
                    ES_TRACE_DEBUG("X_BAND_TRNSM_CMD_GET_Param GETRESULT error %d\r\n", (int)retStat);
                }
#endif
            }
            // Subtract the retry count
...
static uint32_t S_X_Band_Get_Cmd_Timeout(uint16_t Identifier, S_X_BAND_TRNSM_GW_CMD_ID_enum Cmd, uint8_t Type, uint8_t size, const uint8_t *pData)
{
        ...
        case S_X_BAND_TRNSM_GW_CMD_ID_UPLOAD_FILE:
            {
                ...
                fm_result_t fres = fm_stat((const char *)pData, &tfi);

                fsize = (FM_RES_OK == fres) ? tfi.size : 0;
    ...
    ES_TRACE_DEBUG("cmd 0x%04x | type: %c | sz = %lu == timeout (ms) ==> %lu\r\n", Cmd, (char)Type, size, (uint32_t)timeout_ms);
...
static void S_X_Band_On_Dir_List_Start(void)
{
    ...
    (void)fm_close(&moduleCacheInfo.fhnd);
    (void)fm_open(&moduleCacheInfo.fhnd, SX_BAND_FILECACHE_NAME, FM_MODE_CREATE_ALWAYS | FM_MODE_WRITE);
}
...
static void S_X_Band_On_Enumerate_File(const S_X_BAND_TRNSM_FileInfo_struct *const pFileInfo)
{
    if (moduleCacheInfo.in_progress)
    {
        ...
        fm_result_t fres          = fm_write(&moduleCacheInfo.fhnd, pFileInfo, sizeof(S_X_BAND_TRNSM_FileInfo_struct), &bytes_written);

        if ((FM_RES_OK == fres) && (bytes_written == sizeof(S_X_BAND_TRNSM_FileInfo_struct)))
        {
            ES_TRACE_DEBUG("== SXB CACHE UPDATE ===> %s | size = %lu\r\n", pFileInfo->FileName, pFileInfo->Size);
...
static uint64_t S_X_Band_Get_File_Size_From_Cache(const char *const pFilePattern)
{
    ...
    fm_result_t fres = fm_open(&fhnd, SX_BAND_FILECACHE_NAME, FM_MODE_OPEN_EXISTING | FM_MODE_READ);

    if (FM_RES_OK == fres)
    {
        S_X_BAND_TRNSM_FileInfo_struct finfo_entry;

        fres = fm_read(&fhnd, &finfo_entry, sizeof(finfo_entry), &bytes_read);

        while (bytes_read > 0)
        {
            if ((FM_RES_OK == fres) && (bytes_read == sizeof(finfo_entry)))
            {
                if ((finfo_entry.NameLength > 0) &&
                    (file_helper_is_wc_matching(pFilePattern,
                                                strlen(pFilePattern),
