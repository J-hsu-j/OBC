...
sio_fd_t sio_open(u8_t devnum)
{
    ...
    if (devnum < CDEF_ELEMENT_COUNT(sio_descriptor))
    {
        ...
        if (NULL == sio_descriptor[devnum].com_monitor_task_id)
        {
            ...
            task_mon_task_initialized(TASK_ID_SLIP);
            task_mon_register_for_stats(TASK_ID_SLIP, sio_descriptor[devnum].com_monitor_task_id);
        }
...
STATIC void slip_com_recovery_task(void *arg)
{
    ...
    while (true)
    {
        ...
        task_mon_i_am_alive(TASK_ID_SLIP);
        task_mon_low_stack_check(TASK_ID_SLIP);
