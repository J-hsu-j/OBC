...
bool MAC_bStartAllIfs(const eMAC_Interface_InitType_t fp_eInitType)
{
    ...
    for(uint8_t i = 0; i < eMAC_Interface_Count; i++)
    {
        ...
        res &= (uint16_t) MAC_bStart(i, fp_eInitType);
    }
...
ESSATMAC_ErrCodes MAC_DrvEnqMessage(const ESSATMAC_Context * fp_pMACContext, const uint8_t * fp_pMsgData, uint8_t fp_nMsgDataSize)
{
    sMACInterfaceState_t *pMacIntf = NULL;
...
{
    const sMACInterfaceState_t *psInterfaceState = MacCfg_sGetMacInterfaceById(fp_eMacInterfaceId);
    ...
    else
    {
        ...
        bIsPoolEmpty = (0 == psInterfaceState->sDisp.u32RxMsgCount);
    }
...
{
    if((1 == LL_USART_IsEnabledIT_RXNE(fp_psState->psCfg->sHwConfig.psUART)) &&
       (1 == LL_USART_IsActiveFlag_RXNE(fp_psState->psCfg->sHwConfig.psUART)))
    {
    ...
    if((1 == LL_USART_IsEnabledIT_TXE(fp_psState->psCfg->sHwConfig.psUART)) &&
       (1 == LL_USART_IsActiveFlag_TXE(fp_psState->psCfg->sHwConfig.psUART)))
    {
        MAC_IRQ_vTXIndication(fp_psState);
    }

    if((1 == LL_USART_IsEnabledIT_TC(fp_psState->psCfg->sHwConfig.psUART)) &&
       (1 == LL_USART_IsActiveFlag_TC(fp_psState->psCfg->sHwConfig.psUART)))
    {
        LL_USART_ClearFlag_TC(fp_psState->psCfg->sHwConfig.psUART);
        MAC_IRQ_vTXFinishedIndication(fp_psState);
    }

    if((1 == LL_USART_IsEnabledIT_PE(fp_psState->psCfg->sHwConfig.psUART)) &&
       (1 == LL_USART_IsActiveFlag_PE(fp_psState->psCfg->sHwConfig.psUART)))
    {
        LL_USART_ClearFlag_PE(fp_psState->psCfg->sHwConfig.psUART);
        MAC_IRQ_vRXProcessEvent(fp_psState, eRXEvent_RxError);
    }

    if(1 == LL_USART_IsEnabledIT_ERROR(fp_psState->psCfg->sHwConfig.psUART))
    {
        if(1 == LL_USART_IsActiveFlag_NE(fp_psState->psCfg->sHwConfig.psUART))
        {
        ...
        if(1 == LL_USART_IsActiveFlag_FE(fp_psState->psCfg->sHwConfig.psUART))
        {
        ...
        if(1 == LL_USART_IsActiveFlag_ORE(fp_psState->psCfg->sHwConfig.psUART))
        {
    ...
    if(1 == LL_USART_IsActiveFlag_IDLE(fp_psState->psCfg->sHwConfig.psUART))
    {
                              ...
                              eMACInterface_t fp_eInterface, const eMAC_Interface_InitType_t fp_eInitType)
{
    if((!fp_psState->bInitDone) || (fp_eInitType == MACIF_INITTYPE_FULL))
    {
        ...
        {
            (*fp_psState->psCfg->pfUartInit)(&fp_psState->psCfg->sHwConfig);
        }
        ...
        if((fp_psState->psCfg->sHwConfig.u32Baudrate != 0) && (true == fp_psState->bInitDone))
        {
...
STATIC void fs_vDeinitInterface(sMACInterfaceState_t * fp_psState, eMACInterface_t fp_eInterface)
{
    ...
    {
        (*fp_psState->psCfg->pfUartDeinit)(&fp_psState->psCfg->sHwConfig);
    }
