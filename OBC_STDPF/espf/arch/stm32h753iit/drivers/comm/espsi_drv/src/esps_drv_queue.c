...
sMACTXPoolEntry_t *MAC_QUEUE_vTXQueuePop(sMACInterfaceState_t * fp_psMACIntf)
{
    ...
    if(NULL != fp_psMACIntf->sTXCtx.psQueue)
    {
...
{
    if((fp_psEntry != NULL) && (fp_psEntry->pSendCallback != NULL))
    {
        (*fp_psEntry->pSendCallback) (fp_psEntry->nMsgID, (true == fp_bSuccess) ? ESSATMAC_EC_OK : ESSATMAC_EC_RETRY_COUNT_LIMIT);
    }
}
