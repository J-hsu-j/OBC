...
#define NINE_BIT_MASK   ((uint16_t) 0x01FFU)
...
void MAC_IRQ_vTXIndication(sMACInterfaceState_t * fp_psState)
{
    ...
    else
    {
        ...
        LL_USART_TransmitData9(fp_psState->psCfg->sHwConfig.psUART,
                               (uint16_t) (psCmd->pu8TXData[psCmd->u32TXCount] & NINE_BIT_MASK));
...
STATIC void fs_vFullTxDisable(sMACInterfaceState_t * fp_psState)
{
    ...
    if((fp_psState->psCfg->pfDisableTX != NULL) && (true == fp_psState->sTXCtx.bRS485BusDriverEnabled))
    {
