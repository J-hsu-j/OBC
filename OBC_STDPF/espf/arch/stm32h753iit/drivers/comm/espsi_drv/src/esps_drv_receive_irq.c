...
STATIC void fs_vRXState_DispatchMessage(sMACInterfaceState_t * fp_psState)
{
    ...
    if(fp_psState->sRXCtx.psMsgEntry->sMessage.u8SourceAddr != fp_psState->u16Address)
    {
        ...
        if (MacCfg_vUseAltDispatchForFrame(fp_psState->sRXCtx.psMsgEntry) == true)
        {
            MAC_DISP_bDispatchQueueAdd(&fp_psState->sDispAlt, fp_psState->sRXCtx.psMsgEntry);
        }
        else
        {
    ...
    while(fp_psState->sRXCtx.eStateEvent != eRXEvent_None)
    {
        ...
        case (eMACReceive_NewMessage):
            ...
            else
            {
                ...
                // consume the event...
                fp_psState->sRXCtx.eStateEvent = eRXEvent_None;

                // ...call state function explicitly because there is no state change in this case
                fs_vRXState_NewMessage(fp_psState);

                // ...if we have a new buffer available, just start reception again, otherwise
                // we remain stuck in this state
                if (eRXEvent_NewMessage == fp_psState->sRXCtx.eStateEvent)
                {
                    fp_psState->sRXCtx.eState = eMACReceive_WaitAddress;
                }
            }
        ...
        case (eMACReceive_WaitAddress):
            ...
            {
                if (true == MacCfg_Match_Target_Addr(fp_psState->sRXCtx.psMsgEntry->sMessage.u8TargetAddr, fp_psState->macIfId))
                {
        ...
        case (eMACReceive_ReadHeader):
            ...
            {
                if(true == fs_isHeaderCrcValid(fp_psState))
                {
                    fp_psState->sRXCtx.eState = eMACReceive_ReadData;
                }
