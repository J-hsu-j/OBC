...
typedef struct
{
    ...
    uint32_t tx_q_free_cnt;       /**< number of free slots in the TX queue before the next Get */
} can_drv_stats_t;
...
bool can_init(void)
{
    ...
    if (NULL == fs_tx_task_h)
    {
        ...
        task_mon_register_for_stats(TASK_ID_CAN_TX, fs_tx_task_h);
    }
    ...
    if (NULL == fs_init_task_h)
    {
        ...
        task_mon_register_for_stats(TASK_ID_CAN_INIT, fs_init_task_h);
    }
...
static void fs_init_task(void *args)
{
    ...
    task_mon_task_initialized(TASK_ID_CAN_INIT);
    task_mon_set_task_policy(TASK_ID_CAN_INIT, TASK_MON_POLICY_MONITOR);

    while (1)
    {
        task_mon_i_am_alive(TASK_ID_CAN_INIT);
        ...
        {
            ES_TRACE_DEBUG("CAN => [%ld|%ld|%d|%d|%d|%d|%d|%d|%d|%d]",
                           can_drv_stats.rx_cnt,
                           ...
                           can_drv_stats.err_warn_cnt,
                           can_drv_stats.err_bus_off_cnt,
                           can_drv_stats.tx_q_free_cnt);

            timer_start(&stats_timer);
        }

        task_mon_low_stack_check(TASK_ID_CAN_INIT);
    }
}
...
static void fs_tx_task(void *args)
{
    ...
    task_mon_task_initialized(TASK_ID_CAN_TX);
    task_mon_set_task_policy(TASK_ID_CAN_TX, TASK_MON_POLICY_MONITOR);

    while (1)
    {
        task_mon_i_am_alive(TASK_ID_CAN_TX);
        can_drv_stats.tx_q_free_cnt = osMessageQueueGetCount(fs_txq);
        ...
        task_mon_low_stack_check(TASK_ID_CAN_TX);
    }
}
