...
//    
    ...
    struct Eps2Inst{{ eps_bp_instance_id }}_FDIR_CMD_EXEC_STATUS_type
    {
...
//
    ...
    attribute FDIR_EPS_II_BP_INST{{ eps_bp_instance_id }}_CMD_EXEC_STATUS_Data { Status_timeout = 40000 }
{%-endfor%}
"""
...
//
    ...
    **>
    ...
    <** @description: EPS II communication execution status
        @details: id = {{ increment(datacache_data_id) }}
    **>
    attribute Eps2Inst{{ eps_bp_instance_id }}_FDIR_CMD_EXEC_STATUS_type FDIR_EPS_II_BP_INST{{ eps_bp_instance_id }}_CMD_EXEC_STATUS_Data
...
#define EPS_BP_CONFIGURATION                                 \\{% for eps_bp_instance_id in range(config.eps_bp_count) %}
    {                                                        \\
        ...
        .datacache_cmd_list = {                              \\
            ...
            DC_DID_FDIR_EPS_II_BP_INST{{ eps_bp_instance_id }}_CMD_EXEC_STATUS_DATA \\
        },                                                   \\
        .fdir_faults = {                                     \\
            FDIR_FAULT_EPS_INST{{ eps_bp_instance_id }}_CMD_EXEC_FAILURE,           \\
            FDIR_FAULT_EPS_INST{{ eps_bp_instance_id }}_BATT_VOLTAGE_BELOW_SAFE,    \\
            FDIR_FAULT_EPS_INST{{ eps_bp_instance_id }}_BATT_TEMP_BELOW_SAFE,       \\
            FDIR_FAULT_EPS_INST{{ eps_bp_instance_id }}_BATT_TEMP_ABOVE_SAFE        \\
        }                                                    \\
...
nvm_app_defaults_template =  """{%for eps_bp_instance_id in range(config.eps_bp_count)%}\
        {{increment(eps_mac_address)}}{% if not loop.last %},{% endif %} // MAC_ADDR_EPS INST{{ eps_bp_instance_id }}\
{% if not loop.last %}{{"\n"}}{% endif %}
{%-     endfor %}
"""

fdir_srv_cfg_h_template = """{% for eps_bp_instance_id in range(config.eps_bp_count) %}
    FDIR_FAULT_EPS_INST{{ eps_bp_instance_id }}_CMD_EXEC_FAILURE,
    FDIR_FAULT_EPS_INST{{ eps_bp_instance_id }}_BATT_VOLTAGE_BELOW_SAFE,
    FDIR_FAULT_EPS_INST{{ eps_bp_instance_id }}_BATT_TEMP_BELOW_SAFE,
    FDIR_FAULT_EPS_INST{{ eps_bp_instance_id }}_BATT_TEMP_ABOVE_SAFE,
...
fdir_srv_cfg_c_template = """{% for eps_bp_instance_id in range(config.eps_bp_count) %}
    [FDIR_FAULT_EPS_INST{{ eps_bp_instance_id }}_CMD_EXEC_FAILURE]        = "FDIR_FAULT_EPS_INST{{ eps_bp_instance_id }}_CMD_EXEC_FAILURE",
    [FDIR_FAULT_EPS_INST{{ eps_bp_instance_id }}_BATT_VOLTAGE_BELOW_SAFE] = "FDIR_FAULT_EPS_INST{{ eps_bp_instance_id }}_BATT_VOLTAGE_BELOW_SAFE",
    [FDIR_FAULT_EPS_INST{{ eps_bp_instance_id }}_BATT_TEMP_BELOW_SAFE]    = "FDIR_FAULT_EPS_INST{{ eps_bp_instance_id }}_BATT_TEMP_BELOW_SAFE",
    [FDIR_FAULT_EPS_INST{{ eps_bp_instance_id }}_BATT_TEMP_ABOVE_SAFE]    = "FDIR_FAULT_EPS_INST{{ eps_bp_instance_id }}_BATT_TEMP_ABOVE_SAFE",
{%-     endfor %}
"""

fdir_srv_cfg_c_template2 = """{% for eps_bp_instance_id in range(config.eps_bp_count) %}
    [FDIR_FAULT_EPS_INST{{ eps_bp_instance_id }}_CMD_EXEC_FAILURE]        = { .fdir_level = FDIR_EPS_INST{{ eps_bp_instance_id }}_CMD_EXEC_FAILURE_LEVEL,        .p_action_func = NULL,                         .p_escalated_action = NULL                   },
    [FDIR_FAULT_EPS_INST{{ eps_bp_instance_id }}_BATT_VOLTAGE_BELOW_SAFE] = { .fdir_level = FDIR_EPS_INST{{ eps_bp_instance_id }}_BATT_VOLTAGE_BELOW_SAFE_LEVEL, .p_action_func = NULL,                         .p_escalated_action = NULL                   },
    [FDIR_FAULT_EPS_INST{{ eps_bp_instance_id }}_BATT_TEMP_BELOW_SAFE]    = { .fdir_level = FDIR_EPS_INST{{ eps_bp_instance_id }}_BATT_TEMP_BELOW_SAFE_LEVEL,    .p_action_func = NULL,                         .p_escalated_action = NULL                   },
    [FDIR_FAULT_EPS_INST{{ eps_bp_instance_id }}_BATT_TEMP_ABOVE_SAFE]    = { .fdir_level = FDIR_EPS_INST{{ eps_bp_instance_id }}_BATT_TEMP_ABOVE_SAFE_LEVEL,    .p_action_func = NULL,                         .p_escalated_action = NULL                   },
{%-     endfor %}
"""


nvm_app_FDIR_defaults_template =  """{% for eps_bp_instance_id in range(config.eps_bp_count) %}
        [FDIR_FAULT_EPS_INST{{ eps_bp_instance_id }}_CMD_EXEC_FAILURE] = FDIR_EPS_INST{{ eps_bp_instance_id }}_CMD_EXEC_FAILURE_DETECTION_STATE,
        [FDIR_FAULT_EPS_INST{{ eps_bp_instance_id }}_BATT_VOLTAGE_BELOW_SAFE] = FDIR_EPS_INST{{ eps_bp_instance_id }}_BATT_VOLTAGE_BELOW_SAFE_DETECTION_STATE,
        [FDIR_FAULT_EPS_INST{{ eps_bp_instance_id }}_BATT_TEMP_BELOW_SAFE] = FDIR_EPS_INST{{ eps_bp_instance_id }}_BATT_TEMP_BELOW_SAFE_DETECTION_STATE,
        [FDIR_FAULT_EPS_INST{{ eps_bp_instance_id }}_BATT_TEMP_ABOVE_SAFE] = FDIR_EPS_INST{{ eps_bp_instance_id }}_BATT_TEMP_ABOVE_SAFE_DETECTION_STATE,
{%-     endfor %}
"""
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) != 8:
        print(f"Usage: {sys.argv[0]} <eps_bp_count> <datacache_folder> <cfg_code_file> <nvm_eps_fidl_file_path>")
        sys.exit(1)
    ...
    replace_section_in_file(fidl_file_path, j2_fidl_template.render(data),"// begin EPS BP generated code","// end EPS BP generated code")
    replace_section_in_file(fdepl_file_path, j2_fdepl_template.render(data),"// begin EPS BP generated code","// end EPS BP generated code")
    
    
    replace_section_in_file(datacache_fidl_file_path, "","// begin EPS BP generated code","// end EPS BP generated code")
    
    largest_data_id = find_largest_id(datacache_fidl_file_path)
        
    initial_value = [largest_data_id+1]
    
    replace_section_in_file(datacache_fidl_file_path, j2_datacache_fidl_template.render(data, datacache_data_id = initial_value, increment=print_and_increment),"// begin EPS BP generated code","// end EPS BP generated code")
    
    replace_section_in_file(cfg_code_file, j2_bp_cfg_code_template.render(data),"// begin EPS BP generated code","// end EPS BP generated code")
    replace_section_in_file(nvm_eps_fidl_file, j2_obc_fidl_template.render(data),"// begin EPS BP generated code","// end EPS BP generated code")
    
    initial_mac_address = [0x66]
    replace_section_in_file(nvm_app_defs_file, j2_nvm_app_defaults_template.render(data, eps_mac_address = initial_mac_address, increment=print_and_increment),"// begin EPS BP generated code","// end EPS BP generated code")
    replace_section_in_file(nvm_app_defs_file, j2_nvm_app_FDIR_defaults_template.render(data),"// begin EPS BP FDIR generated code","// end EPS BP FDIR generated code")

    replace_section_in_file(fdir_srv_cfg_h_file, j2_fdir_srv_cfg_h_template.render(data),"// begin EPS BP generated code","// end EPS BP generated code")
    
    replace_section_in_file(fdir_srv_cfg_c_file, j2_fdir_srv_cfg_c_template.render(data),"// begin EPS BP generated code","// end EPS BP generated code")
    j2_fdir_srv_cfg_c_template = Template(fdir_srv_cfg_c_template2)
    replace_section_in_file(fdir_srv_cfg_c_file, j2_fdir_srv_cfg_c_template.render(data),"// begin EPS BP generated configuration","// end EPS BP generated configuration")
