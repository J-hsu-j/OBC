...
mod_op_status_id_t eps_i_deinit(sys_init_level_id_t level)
{
    ...
    if (INIT_OS == level)
    {
        task_mon_set_task_policy(TASK_ID_EPS_I, TASK_MON_POLICY_DONT_CARE);
    }
    return STATUS_OK;
}
...
static void eps_i_init_os(void)
{
    ...
    task_mon_register_for_stats(TASK_ID_EPS_I, eps_i_task_handle);

    // Notify task_mon
    task_mon_set_task_policy(TASK_ID_EPS_I, TASK_MON_POLICY_MONITOR);
    task_mon_task_initialized(TASK_ID_EPS_I);
}
...
static void eps_i_task(void *args)
{
    ...
    for (;;)
    {
        ...
        task_mon_i_am_alive(TASK_ID_EPS_I);
        task_mon_low_stack_check(TASK_ID_EPS_I);
    }
}
