...
static uint16_t sds_events_chan_freqs[SDS_SENSTYPECFGNUM_MAX_CNT];
/** @brief Local stash of the events values */
static uint16_t sds_events_chan_events[SDS_SENSTYPECFGNUM_MAX_CNT];
...
/** @brief Deinitialization procedure for a single timer
 *
 *  @param  p_timer Timer handle
 *
 */
STATIC void sds_events_deinit_timer(TIM_HandleTypeDef *const p_timer);
...
void sds_init_events_timer(const sds_sens_read_freq_t *sds_events_cfg)
{
    ...
    for (uint8_t f_num = 0; f_num < SDS_SENSTYPECFGNUM_MAX_CNT; f_num++)
    {
...
{
    sds_events_deinit_timer(&htim3);
    sds_events_deinit_timer(&htim4);
}
...
STATIC void sds_events_delay_elapsed_cb(TIM_HandleTypeDef *htim)
{
    ...
    freq_4 = (&htim3 == htim) ? (0) : (sds_events_chan_freqs[SDS_SENSTYPECFGNUM_SENS_RES_1]);
    ...
    flags_4 = (&htim3 == htim) ? (0) : (sds_events_chan_events[SDS_SENSTYPECFGNUM_SENS_RES_1]);
...
STATIC void sds_events_deinit_timer(TIM_HandleTypeDef *const p_timer)
{
    CRIT_ASSERT(NULL != p_timer);

    if (HAL_TIM_STATE_RESET != HAL_TIM_Base_GetState(p_timer))
    {
        if ((HAL_OK != HAL_TIM_Base_Stop(p_timer)) ||
            (HAL_OK != HAL_TIM_OC_Stop_IT(p_timer, TIM_CHANNEL_1)) ||
            (HAL_OK != HAL_TIM_OC_Stop_IT(p_timer, TIM_CHANNEL_2)) ||
            (HAL_OK != HAL_TIM_OC_Stop_IT(p_timer, TIM_CHANNEL_3)) ||
            (HAL_OK != HAL_TIM_OC_Stop_IT(p_timer, TIM_CHANNEL_4)) ||
            (HAL_OK != HAL_TIM_OC_DeInit(p_timer)) ||
            (HAL_OK != HAL_TIM_Base_DeInit(p_timer)))
        {
            Error_Handler();
        }
    }
