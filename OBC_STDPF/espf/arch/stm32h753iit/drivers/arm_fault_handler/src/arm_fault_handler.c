...
/** @brief Type of fault which occurred last. The variable keeps the value so that the relevant
 *  NVM reset counter handler can be called after the register capture takes place.
 */
static nvm_rc_hard_flt_type_t last_fault = NVM_RC_HARD_FLT_TYPE_MAX;
...
static void __attribute__((used)) capture_registers(uint32_t *const p_fault_stack_frame_start)
{
    ...
    // increase NVM persistent counters which are also used by the bootloader to decide if the operation works adequately
    nvm_rc_hard_fault_cntr_update(last_fault);
...
bool arm_fault_handler_read_last(persisted_ram_x_frame_t *const p_buf)
{
    ...
    if ((NULL != p_buf) && (true == arm_fault_handler_is_frame_valid(&temp_fault_memory_dump)))
    {
...
{
    // store last fault type to be used in the following HANDLE_FAULT() call because it is not expected to
    // return control and we should not call any functions prior to its execution to avoid stack frame
    // corruption
    last_fault = NVM_RC_HARD_FLT_TYPE_MEM;
...
{
    last_fault = NVM_RC_HARD_FLT_TYPE_BUS;
...
{
    last_fault = NVM_RC_HARD_FLT_TYPE_USAGE;
...
{
    last_fault = NVM_RC_HARD_FLT_TYPE_HARD;
