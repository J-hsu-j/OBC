...
void nvm_rc_update(const bool bClearResetReason)
{
    ...
    if (true == __HAL_RCC_GET_FLAG(RCC_FLAG_WWDG1RST)) /* WINDOW_WATCHDOG_RESET */
    {
        BootData.RST_WWD++;
    }

    if (true == __HAL_RCC_GET_FLAG(RCC_FLAG_IWDG1RST)) /* INDEPENDENT_WATCHDOG_RESET */
    {
        BootData.RST_IWD++;
    }

    if (true == __HAL_RCC_GET_FLAG(RCC_FLAG_LPWR1RST)) /* LOW_POWER_RESET */
    {
        BootData.RST_LPR++;
    }

    if (true == __HAL_RCC_GET_FLAG(RCC_FLAG_PORRST)) /* POWER-ON_RESET (POR) / POWER-DOWN_RESET (PDR) */
    {
        BootData.RST_POR++;
    }

    if (true == __HAL_RCC_GET_FLAG(RCC_FLAG_PINRST)) /* EXTERNAL_RESET_PIN_RESET */
    {
        BootData.RST_RstPin++;
    }

    if (true == __HAL_RCC_GET_FLAG(RCC_FLAG_BORRST)) /* BROWNOUT_RESET (BOR) */
    {
        BootData.RST_BOR++;
    }
...
void nvm_rc_hard_fault_cntr_update(const nvm_rc_hard_flt_type_t flt_type)
{
    ...
    switch (flt_type)
    {
        ...
        case NVM_RC_HARD_FLT_TYPE_USAGE:
            {
