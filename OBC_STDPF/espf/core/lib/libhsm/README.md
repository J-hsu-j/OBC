...
- PlantUML standard output support in the code generator to enable SM visualization;
- support of multi-instance SMs.
...
Here is an example state machine code which has only two states which can transition between each other (the code here may vary slightly with recent changes in libhsm):
...
Example_SM:                     # State machine name and top YAML node
    ...
    attributes:                 # named attributes applicable to composite states
        initial: stopped
        instances: 1            # if you skip this, one instance is assumed
```
...
- `instances` - used to specify the number of SM instances to be generated; the attribute is taken into account when specified at top level only;
...
- `__history_<state name>` - can be used as a state name under the `transitions` section; it applies only for Composite states and represents the last active deep history state of the Composite as a target state of a transition, e.g.
...
Example_SM:
    states:
        - stopped:
            transitions:
                - __history_Processing:     # relates to the last active deep history state of the Processing composite state
                    - event: go_back
```
...
### How can I use more than one instance of my SM?
There is not much difference in addressing single- or multi-instance SMs in your code. The main difference is in the generated code which allocates more RAM for the runtime context of each SM instance configured. Since the multi-instance support was not introduced until recently, the legacy **libhsm** APIs have remained backwards compatible and new APIs have been provided to work with more than one instance, e.g. 
- `HSM_TriggerEvent()` remains as it is and operates always on instance 0 of your HSM.
- At the same time, there is the `HSM_TriggerEventForInstance(instance_id, ...)` API which accepts an instance ID as its first argument, keeping the other arguments like in `HSM_TriggerEvent()`.
At some point, the legacy APIs will be deprecated but until then, old code can be built directly without changes using the latest versions of **libhsm**.
It is up to you how you call your different instances of the HSM but you can only have one single implementation for all of them at the moment. If you need to react differently based on the instance, you can pass the instance ID through the HSM event structure.
...
Please create a Jira ticket [here](https://endurosat.atlassian.net/jira/software/c/projects/DH/issues) and set the Jira `Component` field to `libhsm`.