...
def get_pkg_version() -> str:
    """Retrieve the script version from the system. The package must be installed for this to work."""
    v: str = "(unknown)"
    try:
        v = version("hsmgen")
    except PackageNotFoundError as e:
        print(f'The package is not installed on your system and version information is not available! [{e}]')
...
class hsm_file:
        ...
        if os.path.exists(self.file_name):
            ...
            for line in lines:
                ...
                if stripped_line.startswith(hsm_file.END_PREFIX):
                    ...
                    obj_name = stripped_line.removeprefix(hsm_file.START_PREFIX).strip()
                    self.source_objects_dict[obj_name] = ""
                    in_obj_section = True

                    self.jinja_template_lines.append(line)
                    self.jinja_template_lines.append("{{ source_obj_dict[\"" + obj_name + "\"] }}")
                else:
                    self.jinja_template_lines.append(line)
    ...
    def add_removed_object_as_comment(self, obj_name: str, obj_content: str) -> None:
        ...
        self.jinja_template_lines.append('>>>> CODE START"\n')
        ...
        self.jinja_template_lines.append('<<<< CODE END"\n')
        self.jinja_template_lines.append("*/ \n")

    def render_template(self) -> str:
        rtemplate = Environment(loader=BaseLoader).from_string("".join(self.jinja_template_lines))
    ...
    def update_obj(self, obj_name: str, new_obj_content: str) -> bool:
        ...
        if obj_name in self.source_objects_dict:
...
class hsm_merger:
    ...
    def merge(self):
        ...
        with open(self.target_file, "w") as target_file_hnd:
            target_file_hnd.writelines(new_file_obj.render_template())

        print(f':heavy_check_mark: [green]merged output written to {self.target_file}![/green]')
...
class sm:
    ...
    def __init__(self, sm_config: dict):
        ...
        self.states_name_set: set[str] = set()
        self.all_states = []

        self.name = f"{sm_config[0]}"

        for sm_state_config in sm_config[1]["states"]:
            state: sm_state = sm_state.parse_state(self, self.name, sm_state_config)
        ...
        if sm.INSTANCES_ATTR_NAME not in self.attributes:
            self.attributes[sm.INSTANCES_ATTR_NAME] = 1

    def get_state_symbolic_id_for_str(self, base_name : str) -> str:
        return f'{self.name.upper()}_{base_name.upper()}'

    def get_instance_cnt(self):
        return self.attributes[sm.INSTANCES_ATTR_NAME]

    def get_all_states(self) -> list["sm_state"]:
        all_states = []

        for state in self.states_list:
            if state.get_name() not in self.states_name_set:
                self.states_name_set.add(state.get_name())

            all_states.append(state)
            all_states.extend(self.get_all_substates(state))

        return all_states

    def get_all_substates(self, state: "sm_state") -> list["sm_state"]:
        all_substates = []

        if state.is_composite():
            for substate in state.get_substates():
                if substate.is_composite():
                    all_substates.extend(self.get_all_substates(substate))

                if substate.get_name() not in self.states_name_set:
                    self.states_name_set.add(substate.get_name())
    ...
    def check_unique_state_names(self) -> tuple[bool, str]:
        ...
        for state in self.all_states:
            if not state.name in state_dict:
                ...
                res = [False, f'"{state.name}" is used to name more than one state']
                break
        ...
        for state in self.all_states:
            for attr in state.get_attributes().keys():
                if not attr in valid_attr_list:
                    res = [False, f'Invalid attribute "{attr}" specified for state "{state.name}"']
                    break
        ...
        for state in self.all_states:
            for trans in state.get_trans_list():
                if trans.target_state.startswith(HISTORY_STATE_TRANS_PREFIX):
                    target = self.get_state_obj_by_name(trans.target_state.removeprefix(HISTORY_STATE_TRANS_PREFIX))

                    if not target.is_composite():
                        res = [False, f'History state transition not possible for simple state "{target.name}"']
                        break
        return res
    ...
    def get_history_state_info(self) -> set[tuple[str, str]]:
        hist_state_info: set[tuple[str, str]] = set()

        for s in self.all_states:
            for t in s.get_trans_list():
                if t.is_history_state_transition():
                    hist_state_info.add((t.target_state, t.history_target_state))

        return hist_state_info

    @staticmethod
    def get_history_state_id(hist_state_name: str) -> str:
        return f'STATE_{hist_state_name.upper()}'
...
class sm_transition:
    def __init__(self, sm_instance: sm, parent_state: str, trans_record: dict):
        ...
        if self.is_history_state_transition():
            self.history_target_state = self.target_state.removeprefix(sm.SPECIAL_HISTORY_NAME_PREFIX)
        else:
        ...
        if self.target_state.startswith(sm.SPECIAL_TRANSITION_STATE_SELF):
            ...
            self.target_state = self.target_state.removeprefix(sm.SPECIAL_NAME_PREFIX)
...
class sm_state:
    ...
    def get_symbolic_id(self) -> str:
        return f'{self.parent.upper()}_STATE_{self.name.upper()}'
            ...
            for category_name in state_value.keys():
                if category_name == "states":
                    ...
                    for substate in state_value["states"]:
                        state.get_substates().append(sm_state.parse_state(sm_instance, state.name, substate))
                    ...
                    state.attributes = state_descriptor[list(state_descriptor)[0]]["attributes"]
...
class sm_renderer_plantuml(sm_renderer_base):
    def __init__(self, sm_instance: sm):
        ...
        self.history_states_set: set[str] = set()
    ...
    def format_puml_transition(self, state: sm_state, trans: sm_transition) -> str:
        ...
        if trans.is_history_state_transition():
            target_state = f'{trans.history_target_state}_H'
        ...
        guard_cond: str = f'[{trans.get_trans_guard()}]' if trans.get_trans_guard() else ""
        ...
        if trans.is_history_state_transition() and target_state not in self.history_states_set:
            self.history_states_set.add(target_state)
            res = f'state {target_state} as "{trans.history_target_state}[H]" ##[bold]\n{res}'
    ...
    def format_forward_state_decls(self, sm_state_config: dict) -> str:
        ...
        # with a single key named ater the state
        if len(sm_state_config) == 2:
                ...
                for state_value in state_config:
                    ...
                    if "states" in state_value[state_name]:
                        ...
                        for substate in state_value[state_name]["states"]:
                            res = res + self.format_forward_state_decls(substate)
            ...
            if "states" in sm_state_config[list(sm_state_config)[0]]:
                state_config = sm_state_config[list(sm_state_config)[0]]['states']
    ...
    def render_action(self, file_name: str) -> None:
        ...
        self.append_to_out(f"// Generated by hsmgen {get_pkg_version()}")
        self.append_to_out("@startuml")

        self.append_to_out(f"title <b>{self.sm_instance.name}")
        self.append_to_out(f"[*] --> {self.sm_instance.attributes['initial']}")

        self.append_to_out(res)

        processed_transitions_dict: dict[str] = {}

        for state in self.sm_instance.all_states:
            if state.is_composite():
                self.append_to_out(f'state {state.name} {{')
                self.append_to_out(f"[*] --> {state.attributes['initial']}")

                for trans in state.get_trans_list():
                    if not self.get_trans_description(state, trans) in processed_transitions_dict.keys():
                        self.append_to_out(self.format_puml_transition(state, trans))
                        processed_transitions_dict[self.get_trans_description(state, trans)] = 1

                for substate in state.get_substates():
                    for trans in substate.get_trans_list():
                        if not self.get_trans_description(substate, trans) in processed_transitions_dict.keys():
                            self.append_to_out(f'{self.format_puml_transition(substate, trans)}')
                            processed_transitions_dict[self.get_trans_description(substate, trans)] = 1
...
class sm_renderer_jinja(sm_renderer_base):
    def get_std_header_and_version(self) -> str:
        return f"""/*
 ...
 * Any kind of using, copying etc. is prohibited without prior permission.
 * -----------------------------------------------------------------------------
 * Generated by hsmgen {get_pkg_version()}
 * -----------------------------------------------------------------------------
 */"""
    ...
    def render_action(self, file_name: str) -> None:
        outputText = self.template.render(
            hsmgen_std_hdr_w_ver=self.get_std_header_and_version(),
            hsmgen_ver=get_pkg_version(),
            sm=self.sm_instance,
            attributes=self.attributes,
            states=self.sm_instance.all_states,
            hist_states=self.sm_instance.get_history_state_info(),
        )
...
if __name__ == "__main__":
    ...
    for cfg_file in args.conf:
        ...
        for sm_entry in sm_list:
            sm_tree.add(f'{sm_entry.name} [{sm_entry.get_instance_cnt()} instance(s)]')
        ...
        for sm_instance in sm_list:
            if sm_instance.validate_sm_def():
                for sm_instance in sm_list:
                    ...
                    sm_renderer_plantuml(sm_instance).render(os.path.join(args.outpath, f'{sm_inst_name}.puml'))

                    render_dict: dict[str] = {}
                    # render_dict elements are tuples (<file name>, <merge flag if file already exists>)
                    render_dict["hsm_config.c.j2"] = (f"{sm_inst_name}_config.c", False)
                    render_dict["hsm_config_user.c.j2"] = (f"{sm_inst_name}_config_user.c", True)
                    render_dict["hsm_config_user.h.j2"] = (f"{sm_inst_name}_config_user.h", True)
                    render_dict["hsm_user.h.j2"] = (f"{sm_inst_name}_user.h", False)
                    ...
                    for key, value in render_dict.items():
                        ...
                        if (merge_if_file_exists) and (os.path.exists(gen_file_path)):
                            print(f'attempting to merge "{gen_file_path}"...')

                            tmp_new_file_name = f'{gen_file_path}.tmp'

                            # generate to a temporary file to use for the subsequent merge
                            sm_renderer_jinja(sm_instance, args.templatepath, key).render(tmp_new_file_name)

                            # perform merge
                            merger = hsm_merger(gen_file_path, tmp_new_file_name, gen_file_path)
                            merger.merge()
                            ...
                            sm_renderer_jinja(sm_instance, args.templatepath, key).render(gen_file_path)
            else:
                print(f'Code generation for "{sm_instance.name}" interrupted due to errors.')
