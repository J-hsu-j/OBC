...
static ssize_t es_tftp_opkt_size(es_tftp_pkt_t *pkt, ssize_t variable_data_size)
{
    ...
    if (pkt_size > ES_TFTP_PKT_MAX_SIZE)
        pkt_size = -1;
                          ...
                          es_tftp_pkt_data_get_t data_get, void *file_handle, uint32_t *bytes_read)
{
     ...
     */
    (void)data_get((fm_handle_t)file_handle, pkt->data.block, (const uint32_t)ES_TFTP_BLOCK_SIZE, bytes_read);

    if (*bytes_read < 1)
    {
        /* TODO: handle case where 0 bytes are received */
    }

    pkt_size = es_tftp_opkt_size(pkt, (ssize_t)*bytes_read);
    return pkt_size;
}
