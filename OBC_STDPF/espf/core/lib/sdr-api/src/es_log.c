/*  =========================================================================
    es_log - The es_log - SDR API for logging messagess
...
#define es_log_priv_init_default      \
    {                                 \
        ES_WARNING, NULL, NULL, FALSE \
    }
...
void es_log_init(es_log_iface_t *log_iface)
{
    ...
    es_log_priv.enabled   = SDR_LOG_SYS;
}


void es_log_msg(const char *module, int line, int level, const char *format, ...)
{
    if ((!es_log_priv.enabled) || ((es_log_lvl_t)level < es_log_priv.log_lvl))
    {
        return;
    }
    ...
    size = snprintf((char *)buffer, sizeof(buffer), "[%ld] %s (%d) %s: ", timestamp, log_lvl_str[level], line, module);
    ...
    /* Print the message - log service currently attached to obc trace */
    // es_log_priv.tx(buffer, size, ES_LOG_TX_TIMEOUT);
    ES_TRACE_DEBUG("%s", buffer);
...
void es_log_hexdump(const char *module, const char *desc, void *mem, unsigned int len)
{
    ...
    for (i = 0; i < len + ((len % HEXDUMP_COLS) ? (HEXDUMP_COLS - len % HEXDUMP_COLS) : 0); i++)
    {
        ...
        if (i % HEXDUMP_COLS == 0)
        {
            size = snprintf((char *)buffer, LOG_BUFFER_SIZE, "0x%06lx: ", i);
            es_log_priv.tx(buffer, size, ES_LOG_TX_TIMEOUT);
        }
        /* print hex data */
        if (i < len)
