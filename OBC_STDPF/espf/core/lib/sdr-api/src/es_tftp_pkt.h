...
#define ES_TFTP_BLOCK_SIZE        (SDR_CSP_TFTP_BLOCK_SIZE)
#define ES_TFTP_FILENAME_MAX_SIZE (SDR_CSP_TFTP_FILENAME_MAX_SIZE)
#define ES_TFTP_ERR_MSG_MAX_SIZE  (SDR_CSP_TFTP_ERR_MSG_MAX_SIZE)
/* This is the size of the largest packet that we can send/receive */
#define ES_TFTP_PKT_MAX_SIZE      (ES_TFTP_PKT_HEADER_SIZE + ES_TFTP_DATA_HEADER_SIZE + ES_TFTP_BLOCK_SIZE) /* 515 */
...
#define ES_TFTP_BLOCK_N_MAX_VALUE (SDR_CSP_TFTP_BLOCK_N_MAX_VALUE) /* 2^16 */
#define SDR_CSP_MTU               (ES_TFTP_PKT_MAX_SIZE)
...
#pragma GCC diagnostic ignored "-Wpedantic" // Ignore the -pedantic setting because this structure definition
                                            // uses some non-standard extensions
#endif
...
typedef fm_result_t (*es_tftp_pkt_data_get_t)(
    fm_handle_t     fp,
    void *const     data,
...
ssize_t es_tftp_opkt_data(uint8_t *buffer, uint16_t block_number,
                          es_tftp_pkt_data_get_t data_get, void *file_handle, uint32_t *bytes_read);
