...
static fm_file_obj_t infile;
static fm_file_obj_t outfile;
...
void SendBits(unsigned int bits, unsigned int numbits)
{
    ...
    if (bitsin > 16)         /* special case when # bits in buffer exceeds 16 */
    {
        c = bitbuf & 0xFF;
        bw = 0;
        if ((FM_RES_OK != fm_write(&outfile, &c, 1, &bw))
            || (bw < 1))
        {
...
unsigned int ReadBits(unsigned int numbits)
{
    ...
    {
        if ((FM_RES_OK != fm_read(&infile, &c, 1, &br))
            || (br < 1))
        {
...
unsigned int LoadDict(unsigned int dictpos)
{
    unsigned int j;
    uint32_t i;
    static_assert(sizeof(i) == sizeof(unsigned int), "if size of type of i is different from size of unsigned int code must be adjusted");

    i = 0;
    if ((FM_RES_OK != fm_read(&infile, &dict[dictpos], SECTORLEN, &i))
        || (i < 1))
    {
        //end of file
        return 0;
...
void Decompress(void)
{
    ...
    for ( ; ; )
    {
        if (ReadBits(1) == 0)   /* character or match? */
        {
            ...
            {
                bw = 0;
                if ((FM_RES_OK != fm_write(&outfile, &dict[0], DICTSIZE, &bw))
                    || (bw != DICTSIZE))
                {
                    //error writing to output file
                    return;
        ...
        else
        {
            ...
            {
                bw = 0;
                if ((FM_RES_OK != fm_write(&outfile, &dict[0], i, &bw))
                    || (bw != i))
                {
                    //error writing to output file
                    return;
...
int LZ77_Compress(char *name_in, char *name_out, uint32_t timeout)
{
    ...
    (void)memset(&outfile, 0, sizeof(outfile));

    if (FM_RES_OK != fm_open(&infile, name_in, FM_MODE_READ | FM_MODE_OPEN_EXISTING))
    {
        return LZ77_ERROR_FILE;
    }
    if (FM_RES_OK != fm_open(&outfile, name_out, FM_MODE_WRITE | FM_MODE_OPEN_ALWAYS))
    {
        (void)fm_close(&infile);
        return LZ77_ERROR_FILE;
    }
...
int LZ77_Decompress(char *name_in, const char *name_out, uint32_t timeout)
{
    ...
    if (FM_RES_OK != fm_open(&outfile, name_in, FM_MODE_READ | FM_MODE_OPEN_EXISTING))
    {
        (void)fm_close(&infile);
        return LZ77_ERROR_FILE;
    }
