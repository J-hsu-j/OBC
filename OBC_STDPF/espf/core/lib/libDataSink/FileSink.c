...
static void fappend_formatted_buf_va(file_stream_fmt_ctx_t * const p_ctx, const char *format, va_list args)
{
    if (NULL != p_ctx)
    {
        ...
        p_ctx->fop_status_last = fm_write(p_ctx->p_fhnd, p_ctx->p_data_buf, (uint32_t)ch_count, NULL);
    }
}
...
void fstart_multi_buf_seq(file_stream_fmt_ctx_t * const p_ctx)
{
    if (NULL != p_ctx)
    {
        p_ctx->fop_status_last = fm_open(p_ctx->p_fhnd, p_ctx->file_name, FM_MODE_CREATE_ALWAYS | FM_MODE_WRITE);
    }
}
...
void fcommit_multi_buf_seq(file_stream_fmt_ctx_t * const p_ctx)
{
    if (NULL != p_ctx)
    {
        p_ctx->fop_status_last = fm_close(p_ctx->p_fhnd);
    }
}
