...
tle_update_err_t TleUpdateFromFile(const char *file_name)
{
    ...
    if (FM_RES_OK != fm_open(&fileHandle, file_name, FM_MODE_OPEN_EXISTING | FM_MODE_READ))
    {
        return TLE_UPDATE_ERR_FILE;
    }

    (void)memset(tle.line, 0, sizeof(tle.line));
    if (FM_RES_OK != fm_read(&fileHandle, tle.line[0], SGP4_TLE_CHARS_PER_LINE + 4, NULL))
    {
        (void)fm_close(&fileHandle);
        return TLE_UPDATE_ERR_FILE;
    }
    tle.line[0][SGP4_TLE_CHARS_PER_LINE] = 0;

    if (FM_RES_OK != fm_read(&fileHandle, tle.line[1], SGP4_TLE_CHARS_PER_LINE + 4, NULL))
    {
        (void)fm_close(&fileHandle);
        return TLE_UPDATE_ERR_FILE;
    }
