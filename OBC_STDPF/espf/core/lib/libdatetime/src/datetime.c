...
static uint8_t datetime_days_in_month(const datetime_date_t *const p_date)
{
    ...
    if ((true == datetime_is_valid_year(p_date->year)) &&
        (true == datetime_is_valid_month(p_date->mon)))
    {
        if ((true == datetime_is_leap_year(p_date->year)) && (2 == p_date->mon))
        {
...
datetime_dow_t datetime_wday(const datetime_date_t *const p_date)
{
    ...
    if (true == datetime_is_valid_date(p_date))
    {
        // clang-format off
...
unix_time_t datetime_to_unix(const datetime_stamp_t *const p_stamp)
{
    ...
    if ((true == datetime_is_valid_date(&p_stamp->date)) &&
        (true == datetime_is_valid_time(&p_stamp->time)))
    {
        y = p_stamp->date.year;
        m = p_stamp->date.mon;
        d = p_stamp->date.day;
...
unix_time_precise_t datetime_to_unix_precise_now(void)
{
    ...
    unix_time_t         unix_ts = datetime_to_unix(&tstamp);
    unix_time_precise_t result  = {
         .unix_ts = unix_ts,
         .ms      = tstamp.time.ms,
         .us      = tstamp.time.us
    };
