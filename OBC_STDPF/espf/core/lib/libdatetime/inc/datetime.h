...
/** @brief A version of the Unix timestamp which additionally provides sub-second
 *  precision stamp data.
 */
typedef struct
{
    unix_time_t unix_ts; /**< Standard Unix timestamp with 1-second precision */
    uint16_t    ms;      /**< Millisecond part to the second in the Unix TS */
    uint16_t    us;      /**< Microsecond part to the millisecond part of the timestamp */
} PACKED unix_time_precise_t;
...
/**
 * @brief Converts the current system date/time to a UNIX timestamp but also provides additional
 * sub-second precision information to the user.
 *
 * @return UNIX timestamp matching the system date/time setting when the function is called
 *              On error 0 is returned
 */
unix_time_precise_t datetime_to_unix_precise_now(void);
 ...
 *
 * @param [in,out] time pointer to a datetime_time_t structure, which will be decremented
 * @param [in,out] date pointer to a datetime_date_t structure, which will be decremented
 *
