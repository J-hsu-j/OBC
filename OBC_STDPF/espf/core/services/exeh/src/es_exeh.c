...
INIT_CMP_LOGGER(false, TRACE_LVL_WARN)
...
static bool fs_bPersistExceptions(const char *const aEXEHFileName);
...
void EXEH_bStartPersistorTask(void)
{
    ...
    task_mon_register_for_stats(TASK_ID_EXEH_PERSISTOR, EXEHPersistor_TaskHandle);
    task_mon_task_initialized(TASK_ID_EXEH_PERSISTOR);
#endif // #ifdef ENABLE_EXEH_PERSIST
}
...
{
    static char aEXEHFileName[]   = EXEH_DUMP_FILE_NAME;
    static bool bFileNameInitDone = false;

    if ((true == fm_storage_status_ready(FM_VOL_SD_PATH)) && (memcmp(&lastStats, &modStatsTotal, sizeof(sEXEHExceptionsStats)) != 0))
    {
        if (!bFileNameInitDone)
        {
            ...
            for (uint8_t fileId = 0U; fileId < EXEH_DUMP_FN_SLOTS; fileId++)
            {
                ...
                if (FM_RES_OK == fm_stat(aEXEHFileName, &tfi))
                {
                    char aEXEHFileNameTemp[sizeof(EXEH_DUMP_FILE_NAME)];
                    ...
                    (void)fm_delete(aEXEHFileNameTemp);
                    // rename current dump
                    (void)fm_rename(aEXEHFileName, aEXEHFileNameTemp);
        ...
        {
            char aFullName[FM_MAX_PATH];

            snprintf(aFullName, FM_MAX_PATH, "%s%s", EXEH_DUMP_DIRECTORY_NAME, aEXEHFileName);
...
static bool fs_bPersistExceptions(const char *const aEXEHFileName)
{
    ...
    if (aEXEHFileName != NULL)
    {
        ...
        (void)fm_delete(aEXEHFileName);

        fres = fm_open(&fHnd, aEXEHFileName, FM_MODE_WRITE | FM_MODE_CREATE_ALWAYS);
        ...
        if (fres == FM_RES_OK)
        {
            ...
            fres                  = fm_write(&fHnd, &exehFileTempBuf, sizeof(sEXEHDumpFileFormat), &bytesWritten);

            if ((fres == FM_RES_OK) && (bytesWritten != sizeof(sEXEHDumpFileFormat)))
            {
                fres = FM_RES_INT_ERR;
            }
...
static void fs_vEXEHPersistor_Task()
{
    ...
    while (true)
    {
        ...
        task_mon_i_am_alive(TASK_ID_EXEH_PERSISTOR);
        task_mon_low_stack_check(TASK_ID_EXEH_PERSISTOR);
    }
}
#endif // #ifdef ENABLE_EXEH_PERSIST
