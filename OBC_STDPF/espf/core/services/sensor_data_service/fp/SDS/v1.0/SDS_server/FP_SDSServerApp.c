...
{
  .SDS_setSensReadFreqRequestHandler = SDS_setSensReadFreqRequestHandlerImpl,
  .SDS_getSensReadFreqRequestHandler = SDS_getSensReadFreqRequestHandlerImpl,
  .SDS_getSensStateRequestHandler = SDS_getSensStateRequestHandlerImpl,
  .SDS_getSensErrRequestHandler = SDS_getSensErrRequestHandlerImpl,
  .SDS_getSensRegRequestHandler = SDS_getSensRegRequestHandlerImpl,
  .SDS_setSensRegRequestHandler = SDS_setSensRegRequestHandlerImpl
};
            ...
            const SDSgetSensStateRequestData_t* pRequestData)
{
    ...
    if ((pReqCtx != NULL) && (pRequestData != NULL))
    {
        ...
        if ( pRequestData->eSensTypeId < SDS_AVAILABLESENSORS_MAX_CNT)
        {
            ...
            const SDSgetSensErrRequestData_t* pRequestData)
{
    ...
    if ((pReqCtx != NULL) && (pRequestData != NULL))
    {
        ...
        if ( pRequestData->eSensTypeId < SDS_AVAILABLESENSORS_MAX_CNT)
        {
        	...
        	if (SDS_SENSTYPECFGNUM_SENS_GYRO == pRequestData->eSensTypeId)
        	{
                gyroscope_status_bundle_t gyro_status = adis16265_get_status();

                u8X_AXIS = (uint8_t)gyro_status.gyroscope_x_status;
                u8Y_AXIS = (uint8_t)gyro_status.gyroscope_y_status;
                u8Z_AXIS = (uint8_t)gyro_status.gyroscope_z_status;
        	}
        }
        else
        {
            ...
            const SDSgetSensRegRequestData_t* pRequestData)
{
    ...
    if ((pReqCtx != NULL) && (pRequestData != NULL))
    {
        ...
        for (uint8_t s_id = 0; s_id < SDS_AVAILABLESENSORS_MAX_CNT; s_id++)
        {
