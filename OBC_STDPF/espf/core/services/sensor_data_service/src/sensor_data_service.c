...
static osMutexId_t p_sens_lock[SDS_AVAILABLESENSORS_MAX_CNT];
...
#if SDS_PIN_DBG
STATIC void fs_dbg_out_pin_on(uint32_t event_id, SDS_AvailableSensors_t sensor_type);
STATIC void fs_dbg_out_pin_off(uint32_t event_id, SDS_AvailableSensors_t sensor_type);
#endif /* SDS_PIN_DBG */
...
void sds_init(void)
{
    ...
    {
        task_mon_task_initialized(TASK_ID_SDS);
        task_mon_register_for_stats(TASK_ID_SDS, sds_task_taskHandle);
    }

    fs_sds_spawn_locks();

    for (uint8_t sindx = 0; sindx < SDS_AVAILABLESENSORS_MAX_CNT; ++sindx)
    {
        sensor_base_cfg[sindx].sens_if->init();
    }

    events_mask = (uint32_t)pow(2, SDS_AVAILABLESENSORS_MAX_CNT) - 1U;
...
bool sds_sens_lock(SDS_AvailableSensors_t s_type)
{
    bool res = true;
...
bool sds_sens_release(SDS_AvailableSensors_t s_type)
{
    bool res = true;
...
void sds_conv_mag_data_to_sat_frame(sds_mag_data_t const *const p_mag_data, sds_mag_enum_t mag_id, sds_mag_data_sat_t *const p_mag_data_sat)
{
    ...
    double mag_data_sensor_frame[3];
    double mag_data_satellite_frame[3];
    ...
    coordsys_transform_mag(mag_data_sensor_frame, mag_data_satellite_frame);
...
void sds_conv_gyro_data_to_sat_frame(sds_gyro_data_t const *const p_gyro_data, sds_gyro_data_sat_t *const p_gyro_data_sat)
{
    ...
    /* Local arrays for using coordinate frame transformation */
    double gyr_data_sensor_frame[3];
    double gyr_data_satellite_frame[3];

    /* Extract measurements in sensor frame */
    (void)memcpy(gyr_data_sensor_frame, p_gyro_data, sizeof(sds_gyro_data_t));

    /* Transform data from sensor frame to satellite frame */
    coordsys_transform_gyr(gyr_data_sensor_frame, gyr_data_satellite_frame);
...
void sds_conv_css_data_to_sat_frame(sds_css_data_t const *const p_css_data, sds_css_data_sat_t *const p_css_data_sat)
{
    ...
    /* Transform data from sensor frame to satellite frame */
    coordsys_transform_css(css_data_sensor_frame, css_data_satellite_frame);
...
void sds_conv_panel_temp_data_to_sat_frame(sds_panel_temp_data_t const *const p_panel_temp_data, sds_panel_temp_data_sat_t *const p_panel_temp_data_sat)
{
    ...
    /* Local arrays for using coordinate frame transformation */
    double panel_temp_data_sensor_frame[6];
    double panel_temp_data_satellite_frame[6];

    /* Extract measurements in sensor frame */
    (void)memcpy(panel_temp_data_sensor_frame, p_panel_temp_data, sizeof(sds_panel_temp_data_t));

    /* Transform data from sensor frame to satellite frame */
    coordsys_transform_panel_temp(panel_temp_data_sensor_frame, panel_temp_data_satellite_frame);

    /* Fill the satellite frame measurements struct with transformed data */
    (void)memcpy(p_panel_temp_data_sat, panel_temp_data_satellite_frame, sizeof(panel_temp_data_satellite_frame));
}
...
void sds_conv_gyro_temp_data_to_sat_frame(sds_gyro_temp_data_t const *const p_gyro_temp_data, sds_gyro_temp_data_sat_t *const p_gyro_temp_data_sat)
{
    ...
    /* Local arrays for using coordinate frame transformation */
    double gyro_temp_data_sensor_frame[3];
    double gyro_temp_data_satellite_frame[3];

    /* Extract measurements in sensor frame */
    (void)memcpy(gyro_temp_data_sensor_frame, p_gyro_temp_data, sizeof(sds_gyro_temp_data_t));

    /* Transform data from sensor frame to satellite frame */
    coordsys_transform_gyro_temp(gyro_temp_data_sensor_frame, gyro_temp_data_satellite_frame);

    /* Fill the satellite frame measurements struct with transformed data */
    (void)memcpy(p_gyro_temp_data_sat, gyro_temp_data_satellite_frame, sizeof(gyro_temp_data_satellite_frame));
}
...
STATIC void fs_sds_task(void *arg)
{
    ...
    for (;;)
    {
                    ...
                    (true == sds_sens_lock(sindx)))
                {
                    ...
                    if (SDS_SENSOR_STATUS_INIT == sensor_base_cfg[sindx].sens_if->get_state())
                    {
                        if (SDS_OP_STATUS_OK == sds_get_sensor_data(sindx, &sens_data, &sens_data_sat, &p_sens_data, &p_sens_data_sat))
                        {
                            ...
                            dc_set_raw_data(DC_DATA_INPUT_INTERNAL,
                                            sensor_base_cfg[sindx].dc_did_id_SI_body_frame,
                                            p_sens_data_sat, sensor_base_cfg[sindx].dc_did_id_SI_body_frame_size);
                        }
                    }
        ...
        task_mon_i_am_alive(TASK_ID_SDS);
        task_mon_low_stack_check(TASK_ID_SDS);
    }
}
...
STATIC void fs_dbg_out_pin_on(uint32_t event_id, SDS_AvailableSensors_t sensor_type)
{
    //  if ( (SDS_THREAD_FLAGS_MTM == event_id) && (SDS_AVAILABLESENSORS_SENSOR_MAG_PRIMARY == sensor_type) )
    //  {
    //      HAL_GPIO_WritePin(OBC_OUT1_GPIO_Port, OBC_OUT1_Pin, GPIO_PIN_SET);
    //  }

    if ((SDS_THREAD_FLAGS_MTM == event_id) && (SDS_AVAILABLESENSORS_SENSOR_MAG_SECONDARY == sensor_type))
    {
