...
bool eps_ctrl_init(void)
{
    ...
    task_mon_set_task_policy(TASK_ID_EPS_CTRL, TASK_MON_POLICY_MONITOR);
    task_mon_register_for_stats(TASK_ID_EPS_CTRL, h_eps_ctrl_task);
...
void eps_ctrl_deinit(void)
{
    ...
    task_mon_set_task_policy(TASK_ID_EPS_CTRL, TASK_MON_POLICY_DONT_CARE);
...
static void eps_ctrl_task(void *args)
{
    ...
    for (;;)
    {
        ...
        eps_ctrl_cfg_fdir_power_check();

        osDelay(EPS_CTRL_TASK_PERIOD_MS);

        task_mon_i_am_alive(TASK_ID_EPS_CTRL);
        task_mon_low_stack_check(TASK_ID_EPS_CTRL);
    }
}
