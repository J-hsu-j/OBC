...
static DATA_CACHE_SSP_0_Telemetry_t tlm_ssp;
...
mod_op_status_id_t sp_init(sys_init_level_id_t level)
{
    ...
    else if (level == INIT_OS)
    {
        ...
        task_mon_set_task_policy(TASK_ID_SP, TASK_MON_POLICY_MONITOR);
        task_mon_register_for_stats(TASK_ID_SP, h_sp_service_task);
        ret = STATUS_OK;
    }
    else
    {
...
mod_op_status_id_t sp_deinit(sys_init_level_id_t level)
{
    ...
    if (INIT_OS == level)
    {
        task_mon_set_task_policy(TASK_ID_SP, TASK_MON_POLICY_DONT_CARE);
    }
 ...
 */
void sp_response_received(const RespContext_t *p_resp_ctx, const sp_uhf_anttelemetryResponseData_t *p_resp_data)
{
    if (p_resp_ctx->nAddr == ctx.nAddr)
    {
        tlm_ssp.u32Uptime              = p_resp_data->sData.u32Uptime;
        tlm_ssp.u16Sun_data_main       = p_resp_data->sData.sSun_data.u16Sun_data_main;
        tlm_ssp.u8Sun_data_main_valid  = p_resp_data->sData.sSun_data.eSun_vld_main;
        tlm_ssp.u16Sun_data_ext        = p_resp_data->sData.sSun_data.u16Sun_data_ext;
        tlm_ssp.u8Sun_data_ext_valid   = p_resp_data->sData.sSun_data.eSun_vld_ext;
        tlm_ssp.i16Temp_data_main      = p_resp_data->sData.sTemp_data.i16Temp_data_main;
        tlm_ssp.u8Temp_data_main_valid = p_resp_data->sData.sTemp_data.eTemp_vld_main;
        tlm_ssp.i16Temp_data_ext1      = p_resp_data->sData.sTemp_data.i16Temp_data_ext1;
        tlm_ssp.u8Temp_data_ext1_valid = p_resp_data->sData.sTemp_data.eTemp_vld_ext1;
        tlm_ssp.i16Temp_data_ext2      = p_resp_data->sData.sTemp_data.i16Temp_data_ext2;
        tlm_ssp.u8Temp_data_ext2_valid = p_resp_data->sData.sTemp_data.eTemp_vld_ext2;
        tlm_ssp.i16Temp_data_mcu       = p_resp_data->sData.sTemp_data.i16Temp_data_mcu;
        tlm_ssp.u8Temp_data_mcu_valid  = p_resp_data->sData.sTemp_data.eTemp_vld_mcu;
...
static void sp_service_task(void *argument)
{
    ...
    task_mon_task_initialized(TASK_ID_SP);

    while (1)
    {
        task_mon_i_am_alive(TASK_ID_SP);
        ...
        for (uint8_t ssp_index = 0; ssp_index < CDEF_ELEMENT_COUNT(ssp_dc_config); ssp_index++)
        {
            for (uint8_t retry = 0; retry < SP_MAX_RETRIES; retry++)
            {
                ...
                reqStatus = sp_uhf_ant_telemetryReq(&ctx);
        ...
        task_mon_low_stack_check(TASK_ID_SP);
    }
}
