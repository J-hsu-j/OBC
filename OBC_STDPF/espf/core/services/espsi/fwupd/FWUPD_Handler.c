...
static void fs_vFWUPD_Handler_Task(void *argument)
{
    ...
    while (true)
    {
        ...
        {
            aliveCntr = 0U;
            task_mon_i_am_alive(TASK_ID_FWUPD_HANDLER);
            task_mon_low_stack_check(TASK_ID_FWUPD_HANDLER);
        }
    }
}
...
static void fs_vPersistPendingData(bool bUseFullSizedCommits)
{
    ...
    while (bBuffersPending)
    {
        ...
        pData = (true == bUseFullSizedCommits) ? (CircularBuf_ReadNextCommitBuffer(&fwUpdCtx.cbufCtx, &u32DataBytesPending)) : (CircularBuf_GetNextBufferPtr(&fwUpdCtx.cbufCtx, u32BytesRequested, &u32DataBytesPending));
...
static uint8_t fs_vOnESFWUPD_OnNewChunk(uint8_t *pChunk, uint16_t nReceivedSize, uint64_t *pPacketNumber)
{
    ...
    if (bUpdHeaderValid)
    {
        ...
        if (postEvtResult == osOK)
        {
            ...
            else if (res == ECIRCBUFOPRESULT_OK)
            {
...
void FWUPD_Handler_Init(void)
{
    ...
    task_mon_register_for_stats(TASK_ID_FWUPD_HANDLER, FwUpd_TaskHandle);

    task_mon_task_initialized(TASK_ID_FWUPD_HANDLER);
}
