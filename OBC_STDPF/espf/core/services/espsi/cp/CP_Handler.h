...
typedef struct
{
    ...
    uint32_t                     u32UserTimeout;        /**< User timeout value in ms (default timeout used if set to 0) */
...
/** @brief Returns the user-provided timeout value as specified by the dispatch context or the default specified timeout
 *  if user timeout is set to 0.
 *
 *  @param[in] psDispCtx CP dispatcher context
 *  @param[in] u32DefaultTimeout value to use if the provided dispatcher context doesn't have a valid user timeout
 *  specified
 *
 *  @return timeout value in milliseconds
 */
uint32_t CPCmdHandler_GetTimeout(const sCPDispatchContext_t *const psDispCtx, const uint32_t u32DefaultTimeout);
