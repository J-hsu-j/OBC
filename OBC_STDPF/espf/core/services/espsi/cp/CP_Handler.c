        ...
        case ESMTS_WAIT_CMD_PROCESS:
            {
                ...
                if (pfCmdHandler != NULL)
                {
                    ...
                    // let the CP handler calculate its own timeout (this field is used for the comm_gw feature
                    sDispCtx.u32UserTimeout = 0U;
...
{
    BREAK_ASSERT(NULL != psDispCtx);

    return ((NULL != psDispCtx) && (0UL != psDispCtx->u32UserTimeout)) ? (psDispCtx->u32UserTimeout) : u32DefaultTimeout;
}
