...
#define SXBAND_FNAME_SIZE          FM_MAX_PATH               /**< max size of the file names used by the SXBand scheduler implementation */
            ...
            if (psDispCtx->u32CmdDataSize >= sizeof(sSXBandGwCmdHdr_t) + pCmdHdr->u8Size)
            {
                ...
                if (false != ESSA_Stack_TP_Layer_PrepareThreadWait())
                {
                    S_X_BAND_TRNSM_ESTTC_StartCmd(pCmdHdr->Identifier,
                                                  ...
                                                  pCmdHdr->Type,
                                                  pCmdHdr->u8Size,
                                                  (pCmdHdr->u8Size > 0U) ? &psDispCtx->pu8CmdData[sizeof(sSXBandGwCmdHdr_t)] : NULL,
                                                  &userRespCtx,
                                                  &cmd_timeout_ms);
                    ...
                    waitResult = ESSA_Stack_TP_Layer_ThreadWaitForEvent(CPCmdHandler_GetTimeout(psDispCtx,
                                                                                                (cmd_timeout_ms > SXBAND_RESPONSE_TIMEOUT_MS) ? SXBAND_RESPONSE_TIMEOUT_MS : cmd_timeout_ms));
                }
...
typedef struct
{
    uint8_t  file_op_res;   /**< result from the last file operation (maps to FatFS fm_result_t type */
...
/**
 *  @brief Writes the raw bytes of an S/X-band schedule script to a file
 *
 *  @param p_out_args[out] Result of the file operations
 *  @param p_file_name[in] Name of the file to be created
 *  @param p_file_data[in] Bytes to be written
 *  @param data_size[in]   Number of bytes to be written
 */
static void create_sched_file(sxband_sched_cp_out_args_t *const p_out_args,
                              const char *const                 p_file_name,
                              const uint8_t *const              p_file_data,
                              const uint32_t                    data_size)
{
    CRIT_ASSERT((NULL != p_out_args) && (NULL != p_file_name) && (NULL != p_file_data));

    fm_file_obj_t fhnd;
    uint32_t      bw = 0;

    uint8_t file_op_res = (uint8_t)fm_open(&fhnd, p_file_name, FM_MODE_WRITE | FM_MODE_CREATE_ALWAYS);

    if (FM_RES_OK == file_op_res)
    {
        file_op_res = (uint8_t)fm_write(&fhnd, p_file_data, data_size, &bw);

        (void)fm_close(&fhnd);
    }

    p_out_args->file_op_res   = file_op_res;
    p_out_args->bytes_written = bw;
}
    ...
    if (sys_conf_is_module_active(SYS_MODULE_ID_SX_BAND))
    {
        if (CPCmdHandler_IsDispContextValid(psDispCtx, CP_CMD_SXBAND_SCHED_RUN))
        {
            ...
            if (psDispCtx->u32CmdDataSize >= sizeof(sxband_sched_cp_in_args_t))
            {
                ...
                {
                    create_sched_file(&out_args, pCmdHdr->file_name,
                                      &psDispCtx->pu8CmdData[sizeof(sxband_sched_cp_in_args_t)],
                                      psDispCtx->u32CmdDataSize - sizeof(sxband_sched_cp_in_args_t));
                }
