...
void fault_persistor_dump_to_file(const char *p_file_name)
{
    ...
    fm_result_t file_res = FM_RES_DISK_ERR;
    fm_file_obj_t dump_file_hnd;
    fp_iter_ctx_t iter;
    ...
    file_res = fm_open(&dump_file_hnd, p_file_name, FM_MODE_WRITE | FM_MODE_CREATE_ALWAYS);

    if (FM_RES_OK == file_res)
    {
        // add file signature
        (void)fm_write(&dump_file_hnd, "FAULT_LST", 9, NULL);
        // write the record size as a header to the file
        uint32_t rec_size = sizeof(persisted_ram_x_frame_t);
        (void)fm_write(&dump_file_hnd, &rec_size, sizeof(rec_size), NULL);
        ...
        while (fp_iter_next(&iter, &x_frame))
        {
            (void)fm_write(&dump_file_hnd, (void *) &x_frame, sizeof(persisted_ram_x_frame_t), NULL);
        }

        osMutexRelease(fp_lock_id);

        (void)fm_close(&dump_file_hnd);
    }
}
