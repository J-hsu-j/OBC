...
typedef enum
{
    SYS_TIME_CAL_FREQ_F001HZ,
    SYS_TIME_CAL_FREQ_F512HZ,
    SYS_TIME_CAL_FREQ_COUNT
} sys_time_cal_freq_t;

typedef enum
{
    SYS_TIME_CALP_PULSES_RESET,
    SYS_TIME_CALP_PULSES_SET,
    SYS_TIME_CALP_PULSES_COUNT
} sys_time_calp_pulses_t;
...
typedef void (*compensation_function)(sys_date_t* const, sys_time_t* const);
 ...
 * @param[in]  mode Pin output frequency @ref sys_time_cal_freq_t
 * @return     `true` on success, `false` otherwise
 */
 ...
 * @return     `true` on success, `false` otherwise
 */
bool sys_time_set(sys_time_t* const p_time);
 ...
 * @return     `true` on success, `false` otherwise
 */
bool sys_time_set_date(sys_date_t* const p_date);
