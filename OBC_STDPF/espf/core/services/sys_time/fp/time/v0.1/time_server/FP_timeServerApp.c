...
{
  .time_set_dateRequestHandler = time_set_dateRequestHandlerImpl,
  .time_get_dateRequestHandler = time_get_dateRequestHandlerImpl,
  .time_set_timeRequestHandler = time_set_timeRequestHandlerImpl,
  .time_get_timeRequestHandler = time_get_timeRequestHandlerImpl,
  .time_enable_calibration_outputRequestHandler = time_enable_calibration_outputRequestHandlerImpl,
  .time_configure_rtc_calibration_parametersRequestHandler = time_configure_rtc_calibration_parametersRequestHandlerImpl,
  .time_retrieve_rtc_calibration_parametersRequestHandler = time_retrieve_rtc_calibration_parametersRequestHandlerImpl,
  .time_get_rtc_clock_sourceRequestHandler = time_get_rtc_clock_sourceRequestHandlerImpl
};
            ...
            const timeenable_calibration_outputRequestData_t* pRequestData)
{
    ...
    if ((pReqCtx != NULL) && (pRequestData != NULL))
    {
        ...
        static_assert(SYS_TIME_CAL_FREQ_F001HZ == TIME_CAL_FREQ_F001HZ, "Frequency enum values mismatch");
        static_assert(SYS_TIME_CAL_FREQ_F512HZ == TIME_CAL_FREQ_F512HZ, "Frequency enum values mismatch");
        static_assert(SYS_TIME_CAL_FREQ_COUNT == TIME_CAL_FREQ_MAX_CNT, "Frequency enum values mismatch");
        if (sys_time_en_calib_out((sys_time_cal_freq_t)pRequestData->eF))
        {
            eStatus = TIME_OP_STATUS_OK;
        }
        else
            ...
            const timeconfigure_rtc_calibration_parametersRequestData_t* pRequestData)
{
    ...
    if ((pReqCtx != NULL) && (pRequestData != NULL))
    {
        ...
        static_assert(SYS_TIME_CALP_PULSES_RESET == TIME_CALP_PULSES_RESET, "Frequency enum values mismatch");
        static_assert(SYS_TIME_CALP_PULSES_SET == TIME_CALP_PULSES_SET, "Frequency enum values mismatch");
        static_assert(SYS_TIME_CALP_PULSES_COUNT == TIME_CALP_PULSES_MAX_CNT, "Frequency enum values mismatch");
