...
static bool sys_time_set_stamp_raw(sys_date_t *const p_date, sys_time_t *const p_time, compensation_function const compensation_algo);
static bool sys_time_get_stamp_check_isr_flags(sys_date_t *const p_date, sys_time_t *const p_time);

/**
 * @brief fraction type used for RTC time set
 */
typedef double fraction_double_t;
...
bool sys_time_apply_rtc_calib(void)
{
        ...
        (osOK == osMutexAcquire(fs_lock, FS_LOCK_TIME)))
    {
        if (HAL_OK == HAL_RTCEx_SetSmoothCalib(
                          ...
                          (rtc_calib_data.calp == SYS_TIME_CALP_PULSES_SET ? RTC_SMOOTHCALIB_PLUSPULSES_SET : RTC_SMOOTHCALIB_PLUSPULSES_RESET),
                          rtc_calib_data.calm))
        {
            res = true;
        }
...
bool sys_time_en_calib_out(sys_time_cal_freq_t mode)
{
    bool              res      = false;
...
bool sys_time_set(sys_time_t *const p_new_time)
{
    bool       res;
    ...
    res = sys_time_set_stamp(&current_date, p_new_time, NULL, FS_LOCK_TIME);
...
bool sys_time_set_date(sys_date_t *const p_new_date)
{
    bool       res;
    ...
    res = sys_time_set_stamp(p_new_date, &current_time, NULL, FS_LOCK_TIME);
...
bool sys_time_set_stamp(sys_date_t *const p_date, sys_time_t *const p_time, compensation_function const compensation_algo, uint32_t timeout)
{
    bool res = false;
    ...
    {
        res = sys_time_set_stamp_raw(p_date, p_time, compensation_algo);
        (void)osMutexRelease(fs_lock);
    }
 ...
 * @return `true` on success, `false` otherwise
 */
__attribute__((optimize(0))) static bool sys_time_wait_rsf(void)
{
...
static bool sys_time_set_stamp_raw(sys_date_t *const p_date, sys_time_t *const p_time, compensation_function const compensation_algo)
{
    ...
    // execute compensation algorithm
    if (NULL != compensation_algo)
    {
        compensation_algo(p_date, p_time);
    }

    LL_RTC_DisableWriteProtection(fs_hrtc.Instance);

    fraction = LL_RTC_GetSynchPrescaler(fs_hrtc.Instance);
    shift    = fraction - (uint32_t)((p_time->ms * 1e3 + p_time->us) * (fraction_double_t)(fraction + 1) / 1e6);
