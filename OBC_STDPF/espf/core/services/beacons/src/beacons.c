...
static const osMutexAttr_t beacons_lock_attr = {
    .name      = "beacons_mutex",  // human readable mutex name
...
void beacons_start(void)
{
    ...
    if (NULL == beacons_rt_context.os_task_hnd)
    {
        ...
        {
            task_mon_task_initialized(TASK_ID_BEACONS);
            task_mon_register_for_stats(TASK_ID_BEACONS, beacons_rt_context.os_task_hnd);
...
void beacons_confirm_sink_send(const uint16_t id, const bool is_successful_send)
{
    if ((true == is_successful_send) && (beacons_rt_context.last_sink_frame_id == id))
    {
...
STATIC void beacons_task(void *p_task_arg)
{
    ...
    for (;;)
    {
        ...
        task_mon_i_am_alive(TASK_ID_BEACONS);
        task_mon_low_stack_check(TASK_ID_BEACONS);
    }
}
