...
const rlf_cfg_t roll_file_name_cfg = {
    .max_size        = UPY_LOG_MAX_SIZE,
    .pattern         = UPY_LOG_TEMPLATE,
    .pos_marker      = UPY_LOG_MARKER_POS,
    .pos_index       = UPY_LOG_INDEX_POS,
    .max_count       = UPY_LOG_SLOTS,
    .marker_active   = UPY_LOG_ACTIVE_MARKER,
    .marker_inactive = UPY_LOG_INACTIVE_MARKER
};
...
static fm_file_obj_t upy_rlf_file_obj;

static rlf_ctx_t roll_file_name_ctx = {
    .filename                 = "",
    .p_cfg                    = &roll_file_name_cfg,
    .current_index            = 0,
    .handle                   = &upy_rlf_file_obj,
...
void upy_init(void)
{
    ...
    task_mon_task_initialized(TASK_ID_UPYTHON);
    task_mon_set_task_policy(TASK_ID_UPYTHON, TASK_MON_POLICY_DONT_CARE);
...
void upy_task_start(void)
{
    ...
    task_mon_register_for_stats(TASK_ID_UPYTHON, h_upy_task);
}
    ...
    while (true)
    {
                                         ...
                                         UPY_TASK_PERIOD_MS))
        {
            ...
            if (UPY_OK == res)
            {
                ...
                {
                    (void)rlf_open(&roll_file_name_ctx);
                    ...
                    if (UPY_OK == res)
                    {
                        ...
                        else
                        {
                            ...
                            result =
                                upy_run(processsing_script_info.data.pName,
                                        (uint8_t)strnlen(processsing_script_info.data.pName, FM_MAX_PATH));
                            osTimerStop(timeout_timer_id); // the timeout is stopped because the
                                                           // operation is successful
                            uint32_t u32Duration = timer_get_elapsed_time(&h_timer);

                            processsing_script_info.data.eScriptStatus   = result;
                            processsing_script_info.data.u32DurationInMs = u32Duration;

                            res = upy_set_script_status(processsing_script_info.id, &processsing_script_info.data);

                            if (UPY_OK != res)
                            {
                                EXEH_HANDLE_EX(eEXEHSeverity_Error, eEXEHModuleID_UPYTHON, EXEH_UPYTHON_ERROR);
                            }

                            if (NULL != processsing_script_info.data.pfOnSendResultCbk)
                            {
                    ...
                    (void)rlf_close(&roll_file_name_ctx);
                }
                else
                {
                    processsing_script_info.data.eScriptStatus = UPY_ERROR;
        ...
        task_mon_low_stack_check(TASK_ID_UPYTHON);
...
static void timeout_callback(void *argument) // NOSONAR
{
    ...
    if ((NULL != processsing_script_info.data.pfOnSendResultCbk) && (INVALID_SCRIPT_ID != processsing_script_info.id))
    {
...
fm_result_t upy_write_line_to_file(const char *const line)
{
    return rlf_write(&roll_file_name_ctx, line, strlen(line), NULL);
}
