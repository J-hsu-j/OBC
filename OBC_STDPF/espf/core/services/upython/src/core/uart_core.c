...
void mp_hal_stdout_tx_strn(const char *str, mp_uint_t len)
{
    ...
    if ('\0' == line[0])
    {
        ...
        (void)strncpy(line, time_str,
                      MAX_LINE_CHARACTERS - NULL_CHARACHTER_LEN);
    }

    if (MAX_LINE_CHARACTERS < strnlen(line, sizeof(line) - 1) + strnlen(pstr, sizeof(pstr) - 1))
    {
        ES_TRACE_DEBUG(
    ...
    (void)strncat(line, pstr,
                  MAX_LINE_CHARACTERS - strnlen(line, sizeof(line) - 1) - NULL_CHARACHTER_LEN);
