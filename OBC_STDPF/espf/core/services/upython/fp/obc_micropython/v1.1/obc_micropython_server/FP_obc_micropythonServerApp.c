...
{
  .obc_micropython_RequestScriptExecutionRequestHandler = obc_micropython_RequestScriptExecutionRequestHandlerImpl,
  .obc_micropython_GetScriptExecStatusRequestHandler = obc_micropython_GetScriptExecStatusRequestHandlerImpl
};
            ...
            const obc_micropythonRequestScriptExecutionRequestData_t* pRequestData)
{
    ...
    if ((pReqCtx != NULL) && (pRequestData != NULL))
    {
        ...
        eObcPortResult_t res = upy_req_script_execution(
            ...
            (uint8_t)strnlen(pRequestData->strScript_full_name, FF_MAX_LFN), pRequestData->u32Timeout_ms, NULL, &u8Script_id);
