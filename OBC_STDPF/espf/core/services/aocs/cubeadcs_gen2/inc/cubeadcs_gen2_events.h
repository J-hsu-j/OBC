...
/**
 * @brief CubeSpace Gen2 gyro type
 */
typedef enum
{
    CUBEADCS_GEN2_GYRO_TYPE_PRIMARY   = 0, /** @brief  */
    CUBEADCS_GEN2_GYRO_TYPE_REDUNDANT = 1,
} cubeadcs_gen2_gyro_type_t;

typedef enum
{
    CUBEADCS_GEN2_GYRO_CRITICAL_CONTEXT_DRIVER_SETUP_FAILURE     = 0, /** @brief  */
    CUBEADCS_GEN2_GYRO_CRITICAL_CONTEXT_DATA_READY_SETUP_FAILURE = 1,
    CUBEADCS_GEN2_GYRO_CRITICAL_CONTEXT_CONFIGURATION_FAILURE    = 2,
    CUBEADCS_GEN2_GYRO_CRITICAL_CONTEXT_COMATOSE                 = 3,
} cubeadcs_gen2_gyro_critical_context_t;

typedef struct
{
    uint8_t  gyro        : 1; /** @brief The gyro responsible for the event. See @ref cubeadcs_gen2_gyro_type_t. */
    uint8_t  overvoltage : 1; /** @brief If set event was triggered by overvoltage, otherwise it was undervoltage */
    uint8_t  reserved    : 6;
    uint16_t voltage; /** @brief The voltage value that triggered the event. */
} PACKED cubeadcs_gen2_event_gyro_voltage_error_data_t;

typedef struct
{
    uint8_t gyro    : 1; /** @brief The gyro responsible for the event. See @ref cubeadcs_gen2_gyro_type_t. */
    uint8_t context : 7; /** @brief Context for the gyro critical event. See @ref cubeadcs_gen2_gyro_critical_context_t */
} PACKED cubeadcs_gen2_event_gyro_critical_error_data_t;
