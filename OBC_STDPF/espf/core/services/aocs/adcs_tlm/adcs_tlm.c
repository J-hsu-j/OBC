...
void ADCS_TLM_TaskInit(void)
{
    ...
    task_mon_register_for_stats(TASK_ID_ADCS_TLM, ADCS_TLM_Task_TaskHandle);

    task_mon_task_initialized(TASK_ID_ADCS_TLM);
    task_mon_set_task_policy(TASK_ID_ADCS_TLM, TASK_MON_POLICY_MONITOR);
}

void ADCS_TLM_TaskDeInit(void)
{
    task_mon_set_task_policy(TASK_ID_ADCS_TLM, TASK_MON_POLICY_DONT_CARE);
}
...
static void fs_adcs_tlm_task(void *arg)
{
    for (;;)
    {
        ...
        task_mon_i_am_alive(TASK_ID_ADCS_TLM);
        task_mon_low_stack_check(TASK_ID_ADCS_TLM);
    }
}
