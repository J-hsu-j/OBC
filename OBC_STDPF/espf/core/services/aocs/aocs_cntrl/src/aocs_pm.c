...
void aocs_pm_calc_performance(aocs_cntrl_log_data_t *get_data_to_pass_to_pm_algorythm, aocs_cntrl_offset_t *aocs_pm_detect_offset_change, aocs_cntrl_res_t data_status)
{
    ...
    static fm_file_obj_t f_file_handler;                          // file handler for the log file
    static fm_result_t   res = FM_RES_OK;                         // result from the file operations
    ...
    fm_fast_file_info_t  tfi;                                     // file info structure
    uint32_t             size = 0;                                // size for how many bytes to write
    ...
    if (u8_counter_pm >= s_aocs_pm_params.u8_update_interval)
    {
            ...
            (false != s_aocs_pm_cnt.b_auto_logging_enabled))
        {
            ...
            res = fm_stat(c_aocs_pm_file_name, &tfi);

            /* Write  header if the file does not exist */
            if (FM_RES_NO_FILE == res)
            {
                snprintf(c_write_data_buff, sizeof(c_write_data_buff),
                         "date, time, dt,short term metric, data, long term metric, enc history, offset, stable time,"
                         "short term metric, data, long term metric, enc history, offset, stable time,"
                         "short term metric, data, long term metric, enc history, offset, stable time \r\n");
                res = fm_open(&f_file_handler, c_aocs_pm_file_name, FM_MODE_WRITE | FM_MODE_OPEN_EXISTING | FM_MODE_OPEN_APPEND);

                size = strlen(c_write_data_buff);

                if (FM_RES_OK == res)
                {
                    res = fm_write(&f_file_handler, c_write_data_buff, size, &u32_bytes_written);
                    if (FM_RES_OK != res)
                    {
                        EXEH_HANDLE(eEXEHSeverity_Warning, eEXEH_AOCS_PM_FAILED_TO_WRITE_TO_FILE);
                    }
                    fm_close(&f_file_handler);
                }
                else
                {
...
STATIC void fs_aocs_update_att_buff(aocs_pm_ctx_t *context, int32_t input_data, aocs_cntrl_res_t data_status)
{
    ...
    static fm_file_obj_t f_file_handler;                             /* File handler */
    static fm_result_t   res = FM_RES_OK;                            /* File operations result */
    ...
    uint32_t             size = 0;                                   /* Size to write to file */
