...
static aocs_cntrl_types_t used_aocs_type = AOCS_CNTRL_TYPE_MAX;
...
/** @brief Tracks the attempts to set the AOCS system state. */
static uint8_t sys_state_req_retry_counter = 0;
...
STATIC void fs_aocs_cntrl_init_control_mode(void);
STATIC void fs_aocs_cntrl_task_main(void);
...
void aocs_cntrl_task_init(void)
{
    ...
    // Set the type of the underlying AOCS module
    used_aocs_type = get_enabled_aocs_type();

    // The AOCS control task is supposed to be running only when a valid AOCS module has been detected
    CRIT_ASSERT(used_aocs_type < AOCS_CNTRL_TYPE_MAX);
    ...
    if (false != all_correct)
    {
        ...
        task_mon_register_for_stats(TASK_ID_AOCS_CNTRL, fs_h_aocs_cntrl_task_handle);

        task_mon_task_initialized(TASK_ID_AOCS_CNTRL);
        task_mon_set_task_policy(TASK_ID_AOCS_CNTRL, TASK_MON_POLICY_MONITOR);
    }
}

void aocs_cntrl_task_deinit(void)
{
    task_mon_set_task_policy(TASK_ID_AOCS_CNTRL, TASK_MON_POLICY_DONT_CARE);
}
...
bool aocs_cntrl_get_cms_status(aocs_cms_status_t *const p_status)
{
    ...
    {
        uint8_t adcs_status;
        // Out-of-bound access check is needed because this method can be called without
        // a valid configured AOCS module
        if ((AOCS_CNTRL_TYPE_MAX > used_aocs_type) &&
            (AOCS_CNTRL_OK == aocs_cntrl_cfg[used_aocs_type].aocs_if->p_get_cms_status(&adcs_status)))
        {
...
STATIC void fs_aocs_cntrl_task(void *arg)
{
    ...
    for (;;)
    {
        ...
        if (false == fs_adcs_initialized)
        {
            ...
            fs_aocs_cntrl_init_control_mode();
        }
        else
        {
            // Perform the regular activities of the AOCS service
            fs_aocs_cntrl_task_main();
        }
        ...
        task_mon_i_am_alive(TASK_ID_AOCS_CNTRL);
        task_mon_low_stack_check(TASK_ID_AOCS_CNTRL);
    }
}
...
STATIC void fs_aocs_cntrl_task_main(void)
{
    ...
    if ((false != fs_aocs_cntrl_is_new_sys_state_req(&sys_state)) &&
        (false != fs_aocs_cntrl_get_ref_params(&ref_params)))
    {
...
STATIC void fs_aocs_cntrl_set_aocs_dc_data(void)
{
    ...
    aocs_cntrl_wheel_speed_t     meas_wheel_speed  = { .rwl0_speed = (int16_t)0, .rwl1_speed = (int16_t)0, .rwl2_speed = (int16_t)0, .rwl3_speed = (int16_t)0 };
...
STATIC void fs_aocs_cntrl_set_aocs_sys_state_dc_data(bool is_last_req_successful)
{
    ...
    if (AOCS_CNTRL_SYS_STATE_EXISTING_CONTROL == s_aocs_sys_data.u8AdcsSysState)
    {
        sys_state           = AOCS_CNTRL_SYS_STATE_EXISTING_CONTROL;
        sys_state_req_state = AOCS_CNTRL_SYS_STATE_STATE_REQ_COMPLETED;
    }
    else if (true != is_last_req_successful)
    {
        ...
        if (sys_state_req_retry_counter < AOCS_SYS_STATE_REQ_MAX_RETRIES)
        {
            AOCS_CNTRL_TRACE_DBG("%s: RETRY ATTEMPT %d\r", AOCS_CNTRL_HEAD_DBG, sys_state_req_retry_counter + 1);
            sys_state_req_state = (aocs_cntrl_sys_state_req_state_t)s_aocs_sys_data.u8AdcsSysStateStatus;
            sys_state_req_retry_counter++;
        }
        else
        {
            AOCS_CNTRL_TRACE_DBG("%s: ALL RETRIES FAILED\r", AOCS_CNTRL_HEAD_DBG);
            sys_state_req_state         = AOCS_CNTRL_SYS_STATE_STATE_REQ_FAILED;
            sys_state_req_retry_counter = 0;
        }
    }
    ...
    AOCS_CNTRL_TRACE_DBG("%s: SETTING STATE %d with status %d\r", AOCS_CNTRL_HEAD_DBG, sys_state, sys_state_req_state);
