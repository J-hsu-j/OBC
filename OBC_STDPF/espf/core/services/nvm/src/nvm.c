...
Nvm_Init(void)
{
    ...
    if (NVM_Cfg_Init())
    {
        ...
        ES_TRACE_DEBUG("%s init error", NVM_TRACE_SIGNATURE);
}
...
Nvm_Load(void)
{
    ...
    for (uint16_t u16Idx = 0U; u16Idx < NVM_BLOCK_MAX_COUNT; u16Idx++)
    {
        ES_TRACE_DEBUG("%s loading block %d...", NVM_TRACE_SIGNATURE, u16Idx);

        if (NULL == aBlockConfig[u16Idx].pBufDefaults)
        {
            ES_TRACE_DEBUG("FLASH ROM defaults not provided for block id %d!", u16Idx);
        }
...
Nvm_Store(const bool bForceDefaults)
{
    ...
    for (uint16_t u16Idx = 0U; u16Idx < NVM_BLOCK_MAX_COUNT; u16Idx++)
    {
        if (( false != aBlockRTData[u16Idx].bRamMirrorDirty) || bForceDefaults)
        {
            ...
            ES_TRACE_DEBUG("%s Storing block -> %d...", NVM_TRACE_SIGNATURE, u16Idx);
            (void)NvmStoreBlock(
                  ...
                  const uint32_t u32BufSize, const bool bCrcProtected)
{
    ...
    ES_TRACE_DEBUG("%s NvmWriteBlock(%08x) triggered", NVM_TRACE_SIGNATURE,
              u32Address);
