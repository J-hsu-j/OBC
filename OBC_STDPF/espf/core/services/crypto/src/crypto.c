...
typedef struct
{
    char              src_file_name[FM_MAX_PATH];    /**< name of the source file to process */
    char              target_file_name[FM_MAX_PATH]; /**< name of the service-produced file */
...
void crypto_start(void)
{
    ...
    if (NULL == crtc.os_task_hnd)
    {
        ...
        {
            task_mon_task_initialized(TASK_ID_CRYPTO_SRV);
            task_mon_register_for_stats(TASK_ID_CRYPTO_SRV, crtc.os_task_hnd);
        }
    }
}
...
crypto_api_res_t crypto_stream_open_always(crypto_stream_hnd_t *const p_stream_hnd, const char *const p_src_file_name, const cipher_alg_type_t impl_type)
{
    ...
    if (crtc.is_key_empty)
    {
        ...
        if ((NULL != p_stream_hnd) && (NULL != p_src_file_name) && (impl_type < OP_CMOX_MAX))
        {
            ...
            fm_result_t f_src_res = fm_open(&p_stream_hnd->f_src_hnd, p_src_file_name, FM_MODE_OPEN_EXISTING | FM_MODE_READ);

            if (FM_RES_OK != f_src_res)
            {
            ...
            (void)fm_size(&p_stream_hnd->f_src_hnd, &file_size);
            p_stream_hnd->encr_block_hdr.cipher_text_size = (uint64_t)file_size;
...
void crypto_stream_close(crypto_stream_hnd_t *const p_stream_hnd)
{
    ...
    (void)fm_close(&p_stream_hnd->f_src_hnd);
...
STATIC crypto_req_status_t queue_cipher_op(const char *const p_src_file_name, const char *const p_target_file_name, const p_crypto_notif p_user_cbk, cipher_alg_type_t alg_type)
{
    ...
    fm_result_t         fres = fm_stat(p_src_file_name, &tfi);
    if ((NULL == p_src_file_name) || (FM_RES_NO_FILE == fres) || (FM_RES_NO_PATH == fres))
    {
...
STATIC void crypto_task(void *p_task_arg)
{
    ...
    for (;;)
    {
        ...
        task_mon_i_am_alive(TASK_ID_CRYPTO_SRV);
        task_mon_low_stack_check(TASK_ID_CRYPTO_SRV);
    }
}
...
STATIC crypto_api_res_t crypt_decrypt_file(const char *const p_src_file_name, const char *const p_target_file_name, const cipher_alg_type_t impl_type)
{
    ...
    if ((NULL != p_src_file_name) && (NULL != p_target_file_name) && (impl_type < OP_CMOX_MAX))
    {
        ...
        fm_result_t      f_target_res = fm_open(&target_file_hnd, p_target_file_name, FM_MODE_CREATE_ALWAYS | FM_MODE_WRITE);
        ...
        if (FM_RES_OK == f_target_res)
        {
            while (CRYPTO_API_RES_OK == stream_res)
            {
                ...
                if ((CRYPTO_API_RES_OK == stream_res) || (CRYPTO_API_RES_EOS == stream_res))
                {
                    ...
                    f_target_res = fm_write(&target_file_hnd, tmp_cipher_buf, bytes_read, &bytes_written);

                    if ((FM_RES_OK != f_target_res) || (bytes_written != bytes_read))
                    {
...
STATIC crypto_api_res_t prep_file_for_stream_encr(crypto_stream_hnd_t *const p_stream_hnd)
{
    ...
    if (NULL != p_stream_hnd)
    {
        ...
        uint32_t text_size;
        fm_size(&p_stream_hnd->f_src_hnd, &text_size);
        p_stream_hnd->encr_block_hdr.cipher_text_size = (uint64_t)text_size;
...
STATIC crypto_api_res_t prep_file_for_stream_decr(crypto_stream_hnd_t *const p_stream_hnd)
{
    ...
    fm_result_t f_src_res = fm_read(&p_stream_hnd->f_src_hnd, &p_stream_hnd->encr_block_hdr, sizeof(encrypted_data_hdr_t), &bytes_read);

    if ((FM_RES_OK != f_src_res) || (bytes_read != sizeof(encrypted_data_hdr_t)))
    {
    ...
    else
    {
        ...
        else
        {
            ...
            f_src_res = fm_seek(&p_stream_hnd->f_src_hnd, p_stream_hnd->encr_block_hdr.cipher_text_offset);
...
STATIC crypto_api_res_t prepare_temp_buf(crypto_stream_hnd_t *const p_stream_hnd, uint8_t *p_read_buf, uint8_t *const p_output_buf, const uint32_t bytes_to_read, uint32_t *const p_bytes_read)
{
    ...
    // (this is important for the decryption step because fm_read() will read after the cipher text block, crossing over to the
    // footer block... and we don't want to process the footer block data as cipher text);
    // in the case of streaming encryption, fm_read() will be reading from the original file and it cannot go beyond the end of the file,
    // thus no issue there
    ...
    fm_result_t f_src_res = fm_read(&p_stream_hnd->f_src_hnd, p_read_buf, actual_bytes_to_read, &bytes_read);
...
STATIC crypto_api_res_t verify_footer(crypto_stream_hnd_t *const p_stream_hnd, uint8_t *p_read_buf, uint8_t *const p_output_buf, const uint32_t bytes_to_read, uint32_t *const p_bytes_read)
{
    ...
    *p_bytes_read = 0U;
    ...
    (void)fm_read(&p_stream_hnd->f_src_hnd, &p_stream_hnd->data_footer, sizeof(p_stream_hnd->data_footer), &bytes_read);
...
STATIC crypto_api_res_t read_from_file(crypto_stream_hnd_t *const p_stream_hnd, uint8_t *p_read_buf, uint8_t *const p_output_buf, const uint32_t bytes_to_read, uint32_t *const p_bytes_read)
{
    ...
    *p_bytes_read = 0;
    ...
    if (p_stream_hnd->cipher_text_bytes_read < p_stream_hnd->encr_block_hdr.cipher_text_size)
    {
        ...
        fm_result_t f_src_res = fm_read(&p_stream_hnd->f_src_hnd, p_output_buf, actual_bytes_to_read, &bytes_read);
