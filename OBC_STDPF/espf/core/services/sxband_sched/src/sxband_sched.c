...
{
    task_mon_set_task_policy(TASK_ID_SXBAND_SCHED, TASK_MON_POLICY_DONT_CARE);
}
...
void sxband_sched_start(void)
{
    ...
    if (NULL == sxband_sched_rt_context.os_task_hnd)
    {
        ...
        {
            task_mon_task_initialized(TASK_ID_SXBAND_SCHED);
            task_mon_register_for_stats(TASK_ID_SXBAND_SCHED, sxband_sched_rt_context.os_task_hnd);
        }
    }
    ...
    if (STATE_IDLE == p_hsm_active_state->state_id)
    {
        ...
        else
        {
            if (NULL != p_sched_notif)
            {
                ...
                (void)fm_open(&def_fhnd, DEFAULT_SUMMARY_FILE_NAME, FM_MODE_CREATE_ALWAYS | FM_MODE_WRITE);
                (void)fm_write(&def_fhnd, err_buf, char_count, NULL);
                (void)fm_close(&def_fhnd);
...
STATIC void sxband_sched_task(void *p_task_arg)
{
    ...
    for (;;)
    {
        ...
        task_mon_i_am_alive(TASK_ID_SXBAND_SCHED);
        task_mon_low_stack_check(TASK_ID_SXBAND_SCHED);
    }
}
