...
static bool read_data_and_verify(sxband_cmd_file_iter_ctx_t * const p_file_ctx, void * const p_data, const uint32_t exp_size)
{
    ...
    p_file_ctx->fs_last_res = fm_read(&p_file_ctx->f_hnd, p_data, exp_size, &bytes_read);

    return (FM_RES_OK == p_file_ctx->fs_last_res) && (bytes_read == exp_size);
}
...
sxband_cmd_iter_init_status_t sxband_cmd_iter_file_init_ctx(const char * const cmd_file_name, const uint8_t cmd_file_name_size, sxband_cmd_file_iter_ctx_t * const p_file_ctx)
{
    ...
    if (NULL != p_file_ctx)
    {
        ...
        (void)fm_close(&p_file_ctx->f_hnd);
        ...
        p_file_ctx->fs_last_res = fm_open(&p_file_ctx->f_hnd, p_file_ctx->cmd_file_name, FM_MODE_READ | FM_MODE_OPEN_EXISTING);

        if (FM_RES_OK == p_file_ctx->fs_last_res)
        {
...
pl_ctrl_iter_status_t file_iter_reset(void * const p_ctx)
{
    ...
    if (NULL != p_ctx)
    {
        ...
        p_file_ctx->fs_last_res = fm_seek(&p_file_ctx->f_hnd, sizeof(cmd_list_hdr_t));
...
bool file_iter_has_next(void * const p_ctx)
{
    ...
    if (NULL != p_ctx)
    {
        ...
        (void)fm_tell(&p_file_ctx->f_hnd, &current_position);
        (void)fm_size(&p_file_ctx->f_hnd, &file_size);
        has_next = (FM_RES_OK == p_file_ctx->fs_last_res) ? (current_position < file_size) : false;
    }
