...
typedef struct
{
    fm_file_obj_t rep_file_hnd;     /**< report file FS handle */
...
static bool copy_sxband_status_to_report(fm_handle_t report_file_hnd);
static inline bool write_and_verify(fm_handle_t fhnd, const uint8_t *p_buf, const uint32_t buf_size);
...
static bool copy_sxband_status_to_report(fm_handle_t report_file_hnd)
{
    ...
    if (report_rt_ctx.file_opened)
    {
        ...
        fm_file_obj_t sxband_log_hnd;
        fm_result_t fres = fm_open(&sxband_log_hnd, SX_BAND_FOUTPUT_NAME, FM_MODE_READ);
        uint32_t bytes_read = 0U;
        bool write_ok = false;

        if (FM_RES_OK == fres)
        {
            fres = fm_read(&sxband_log_hnd, copy_buf, sizeof(copy_buf), &bytes_read);
...
static inline bool write_and_verify(fm_handle_t fhnd, const uint8_t *p_buf, const uint32_t buf_size)
{
    ...
    fm_result_t fres = fm_write(fhnd, (const void *)p_buf, buf_size, &bytes_written);

    return (FM_RES_OK == fres) && (bytes_written == buf_size);
}
...
void sxband_sched_report_open(const char * const p_fname)
{
    ...
    (void)fm_delete(TEMP_REPORT_FNAME);
    report_rt_ctx.p_actual_rep_fname = p_fname;

    fm_result_t fres = fm_open(&report_rt_ctx.rep_file_hnd, TEMP_REPORT_FNAME, FM_MODE_CREATE_ALWAYS | FM_MODE_WRITE);

    if (FM_RES_OK == fres)
    {
        uint32_t bytes_written = 0U;

        fres = fm_write(&report_rt_ctx.rep_file_hnd, CSV_HEADER_COL_NAMES, sizeof(CSV_HEADER_COL_NAMES) - 1U, &bytes_written);

        if ((FM_RES_OK == fres) && (bytes_written == sizeof(CSV_HEADER_COL_NAMES) - 1U))
        {
            report_rt_ctx.file_opened = true;
        }
        else
        {
            (void)fm_close(&report_rt_ctx.rep_file_hnd);
...
void sxband_sched_report_append(const sxband_sched_cmd_t * const p_cmd, const sxband_sched_cmd_status_id cmd_user_status)
{
    ...
    if (report_rt_ctx.file_opened)
    {
        ...
        switch (cmd_user_status)
        {
            ...
            {
                (void)fm_write(&report_rt_ctx.rep_file_hnd, TIMEOUT_STR, sizeof(TIMEOUT_STR) - 1U, NULL);
            ...
            {
                (void)fm_write(&report_rt_ctx.rep_file_hnd, NO_FILES_STR, sizeof(NO_FILES_STR) - 1U, NULL);
            ...
            default:
            {
                if (!copy_sxband_status_to_report(&report_rt_ctx.rep_file_hnd))
                {
                    ...
                    (void)fm_write(&report_rt_ctx.rep_file_hnd, ERROR_STR, sizeof(ERROR_STR) - 1U, NULL);
                }
        ...
        (void)fm_sync(&report_rt_ctx.rep_file_hnd);
...
void sxband_sched_report_append_free_text(const char * const p_text, const uint16_t text_size)
{
    ...
    if (report_rt_ctx.file_opened)
    {
        (void)fm_write(&report_rt_ctx.rep_file_hnd, p_text, text_size, NULL);
    }
}
...
void sxband_sched_report_close(void)
{
    if (report_rt_ctx.file_opened)
    {
        ...
        (void)fm_delete(report_rt_ctx.p_actual_rep_fname);

        // rename the temporary report to the actual name requested by the user
        if (FM_RES_OK != fm_rename(TEMP_REPORT_FNAME, report_rt_ctx.p_actual_rep_fname))
        {
