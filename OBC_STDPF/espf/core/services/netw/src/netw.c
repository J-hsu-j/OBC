...
static void netif_config(void)
{
#if (defined(ETHERNET_SUPPORT_ENABLED) || defined(SLIP_SUPPORT_ENABLED))
    ip_addr_t ipaddr;
    ip_addr_t netmask;
    ip_addr_t gw;
...
#if defined(SLIP_SUPPORT_ENABLED)
    task_mon_task_initialized(TASK_ID_NETW);
    task_mon_register_for_stats(TASK_ID_NETW, netw_task_hnd);
    while (1)
    {
        sio_netif_iter_begin();
