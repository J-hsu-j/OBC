...
ErrorCode cubeObc_cubeComputer_pollForFtpState(TypeDef_TctlmEndpoint *endpoint,
											   TypesCubeComputerControlProgram8_FtpState state,
											   U32 backoff, U32 timeout,
											   TypesCubeComputerControlProgram8_FileTransferStatus *status)
{
	ErrorCode result;
	Boolean done = FALSE;
	...
	do
	{
	    if (true == cubeObc_critical_enter(CUBEOBC_CRITICAL_WAIT_FOREVER))
	    {
...
ErrorCode cubeObc_cubeComputer_pollForFtpUpgradeState(TypeDef_TctlmEndpoint *endpoint,
													  TypesCubeComputerControlProgram8_FtpUpgradeState state,
													  U32 backoff, U32 timeout,
													  TypesCubeComputerControlProgram8_FileTransferStatus *status)
{
	ErrorCode result;
												   ...
												   TypesCubeComputerControlProgram8_FileTransferStatus *status)
{
	...
	U16 metaSize;
	U32 dataSize;
	TypesCubeComputerControlProgram8_FileTransferSetup setup;
	U8 *data;
	...
	if (result == CUBEOBC_ERROR_OK)
	{
		...
		setup.opCode = TYPES_CUBE_COMPUTER_CONTROL_PROGRAM_8__FTP_UPLOAD;
	...
	if (result == CUBEOBC_ERROR_OK)
	{
		...
		result = cubeObc_cubeComputer_pollForFtpState(endpoint, TYPES_CUBE_COMPUTER_CONTROL_PROGRAM_8__BUSY,
													  50u, 1000u, status);
...
ErrorCode cubeObc_cubeComputer_upgrade(TypeDef_TctlmEndpoint *endpoint,
									   TypesCommonFrameworkTypes1_NodeType nodeType, U32 serialInt,
									   TypesCommonFrameworkTypes1_ProgramType program,
									   TypesCubeComputerControlProgram8_NodePort forcePort,
									   TypesCubeComputerControlProgram8_FileTransferStatus *status)
{
	if ((endpoint == NULL) || (status == NULL))
		return CUBEOBC_ERROR_NULLPTR;
	...
	if (endpoint->nodeType != TYPES_COMMON_FRAMEWORK_TYPES_1__NODE_TYPE_CUBE_COMPUTER)
		return CUBEOBC_ERROR_NODE_TYPE;

	if (forcePort != TYPES_CUBE_COMPUTER_CONTROL_PROGRAM_8__PORT_NONE)
	{
		// Force port upgrades do not apply to upgrading CubeComputer itself
		if (nodeType == TYPES_COMMON_FRAMEWORK_TYPES_1__NODE_TYPE_CUBE_COMPUTER)
		...
		if (program != TYPES_COMMON_FRAMEWORK_TYPES_1__PROGRAM_TYPE_BOOTLOADER)
			return CUBEOBC_ERROR_USAGE;
	}
	...
	setup.opCode = TYPES_CUBE_COMPUTER_CONTROL_PROGRAM_8__FTP_UPGRADE;
	setup.node = nodeType;
	...
	if (result == CUBEOBC_ERROR_OK)
	{
		result = cubeObc_cubeComputer_pollForFtpUpgradeState(endpoint, TYPES_CUBE_COMPUTER_CONTROL_PROGRAM_8__UPGRADE_IDLE,
															 500u, 120000u, status);
...
ErrorCode cubeObc_cubeComputer_getFileInfo(TypeDef_TctlmEndpoint *endpoint,
										   TypesCubeComputerControlProgram8_FtpFiles file,
										   TypesCubeComputerControlProgram8_FileInfo *info,
										   TypesCubeComputerControlProgram8_FileTransferStatus *status)
{
	...
	setup.opCode = TYPES_CUBE_COMPUTER_CONTROL_PROGRAM_8__FTP_INFO;
	setup.file = file;
	...
	if (result == CUBEOBC_ERROR_OK)
	{
		result = cubeObc_cubeComputer_pollForFtpState(endpoint, TYPES_CUBE_COMPUTER_CONTROL_PROGRAM_8__IDLE,
													  50u, 3000u, status);
...
ErrorCode cubeObc_cubeComputer_imagePollForState(TypeDef_TctlmEndpoint *endpoint,
												 TypesCubeComputerCommon3_ImageTransferState state,
												 U32 backoff, U32 timeout,
												 TypesCubeComputerCommon3_ImageTransferStatus *status)
{
	ErrorCode result;
	Boolean done = FALSE;
	U16 backoffTotal = 0u; // How long we have waited in total for initialization to complete
...
ErrorCode cubeObc_cubeComputer_imageGetInfoFirstLast(TypeDef_TctlmEndpoint *endpoint,
													 TypesCubeComputerCommon3_ImageFileInfo *firstInfo,
													 TypesCubeComputerCommon3_ImageFileInfo *lastInfo)
{
	if ((endpoint == NULL) || (firstInfo == NULL) || (lastInfo == NULL))
		return CUBEOBC_ERROR_NULLPTR;

	ErrorCode result;
	TypesCubeComputerCommon3_ImageTransferSetup setup;

	ZERO_VAR(*firstInfo);
	ZERO_VAR(*lastInfo);
	ZERO_VAR(setup);

	setup.opCode = TYPES_CUBE_COMPUTER_COMMON_3__INFO_RESET;
	...
	if (result == CUBEOBC_ERROR_OK)
	{
		...
		do
		{
			TypesCubeComputerCommon3_ImageFileInfo info;
			U32 retry = 10u;
...
ErrorCode cubeObc_cubeComputer_imageGetInfo(TypeDef_TctlmEndpoint *endpoint,
											U32 fileHandle, TypesCubeComputerCommon3_ImageFileInfo *info)
{
	...
	if (result == CUBEOBC_ERROR_OK)
	{
		...
		do
		{
			TypesCubeComputerCommon3_ImageFileInfo imgInfo;
			U32 retry = 10u;
...
ErrorCode cubeObc_cubeComputer_imageCaptureStore(TypeDef_TctlmEndpoint *endpoint,
												 TypesCommonFrameworkTypes1_AbstractNode nodeType,
												 TypesCubeComputerCommon3_ImageTransferStatus *status)
{
	if ((endpoint == NULL) || (status == NULL))
		return CUBEOBC_ERROR_NULLPTR;
	...
	setup.opCode = TYPES_CUBE_COMPUTER_COMMON_3__CAPTURE_STORE;
	...
	if (result == CUBEOBC_ERROR_OK)
	{
		...
		result = cubeObc_cubeComputer_imagePollForState(endpoint, TYPES_CUBE_COMPUTER_COMMON_3__STATE_BUSY_STORE, 10u, 5000u, status);
		...
		if (result == CUBEOBC_ERROR_OK)
		{
			...
			result = cubeObc_cubeComputer_imagePollForState(endpoint, TYPES_CUBE_COMPUTER_COMMON_3__STATE_IDLE, 50u, 120000u, status);
...
ErrorCode cubeObc_cubeComputer_imageDownloadDirect(TypeDef_TctlmEndpoint *endpoint,
												   TypesCommonFrameworkTypes1_AbstractNode nodeType, void *userData,
												   TypesCubeComputerCommon3_ImageTransferStatus *status)
{
	if ((endpoint == NULL) || (status == NULL))
		return CUBEOBC_ERROR_NULLPTR;
	...
	setup.opCode = TYPES_CUBE_COMPUTER_COMMON_3__CAPTURE_DOWNLOAD;
	setup.nodeType = nodeType;
	...
	if (result == CUBEOBC_ERROR_OK)
	{
		result = cubeObc_cubeComputer_imagePollForState(endpoint, TYPES_CUBE_COMPUTER_COMMON_3__STATE_BUSY_DOWNLOAD, 50u, 5000u, status);
												   ...
												   TypesCubeComputerCommon3_ImageTransferStatus *status)
{
	...
	setup.opCode = TYPES_CUBE_COMPUTER_COMMON_3__DOWNLOAD;
...
ErrorCode cubeObc_cubeComputer_eventPollForState(TypeDef_TctlmEndpoint *endpoint,
												 TypesCubeComputerCommon3_EventReadQueueState state,
												 U32 backoff, U32 timeout,
												 TypesCubeComputerCommon3_EventLogStatus *status)
{
	ErrorCode result;
	Boolean done = FALSE;
	U16 backoffTotal = 0u; // How long we have waited in total for initialization to complete
...
ErrorCode cubeObc_cubeComputer_eventDownload(TypeDef_TctlmEndpoint *endpoint,
											 TypesCubeComputerCommon3_EventLogFilterTransferSetup *setup, void *userData,
											 TypesCubeComputerCommon3_EventLogStatus *status)
{
	if ((endpoint == NULL) || (status == NULL))
		return CUBEOBC_ERROR_NULLPTR;
	...
	if (result == CUBEOBC_ERROR_OK)
	{
		result = cubeObc_cubeComputer_eventPollForState(endpoint, TYPES_CUBE_COMPUTER_COMMON_3__EVT_READ_QDOWNLOAD,
														100u, 10000u, status);
	}
...
ErrorCode cubeObc_cubeComputer_tlmIncludeLogId(U8 logId, TypesCubeComputerCommon3_TelemetryLogTransferSetup *setup)
{
...
ErrorCode cubeObc_cubeComputer_tlmPollForState(TypeDef_TctlmEndpoint *endpoint,
											   TypesCubeComputerCommon3_TlmLogReadQueueState state,
											   U32 backoff, U32 timeout,
											   TypesCubeComputerCommon3_TelemtryLogStatus *status)
{
	ErrorCode result;
	Boolean done = FALSE;
	U16 backoffTotal = 0u; // How long we have waited in total for initialization to complete
...
ErrorCode cubeObc_cubeComputer_tlmDownload(TypeDef_TctlmEndpoint *endpoint,
										   TypesCubeComputerCommon3_TelemetryLogTransferSetup *setup, void *userData,
										   TypesCubeComputerCommon3_TelemtryLogStatus *status)
{
	if ((endpoint == NULL) || (status == NULL))
		return CUBEOBC_ERROR_NULLPTR;
	...
	if (result == CUBEOBC_ERROR_OK)
	{
		result = cubeObc_cubeComputer_tlmPollForState(endpoint, TYPES_CUBE_COMPUTER_COMMON_3__TLM_READ_QDOWNLOAD,
													  100u, 10000u, status);
	}
