...
{
    task_mon_i_am_alive(TASK_ID_NVM);
    task_mon_low_stack_check(TASK_ID_NVM);
}

bool NVM_Cfg_WriteBlock(uint32_t u32Address, uint8_t *const pu8BlockData,
                        uint32_t u32BlockSize)
{
    return FRAM_WriteData(u32Address, u32BlockSize, (uint8_t *)pu8BlockData);
}

bool NVM_Cfg_ReadBlock(uint32_t u32Address, uint8_t *const pu8BlockData,
                       uint32_t u32BlockSize)
{
