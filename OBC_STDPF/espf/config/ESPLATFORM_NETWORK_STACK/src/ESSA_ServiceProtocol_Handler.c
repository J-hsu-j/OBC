...
void ESSASP_CB_PrepareShipMode(ESSASP_ReplyContext *pRC)
{
    ...
    bool rstCntrsRes = nvm_rc_clear(RSTCOUNTERID_ALL);

    // format all SD cards
    fm_result_t fr = fm_format(FM_VOL_SD_PATH);

    psmRes = ((fr == FM_RES_OK) && (rstCntrsRes)) ? (ESSASPNE_SUCCESS) : (ESSASPNE_GENERAL_ERROR);
