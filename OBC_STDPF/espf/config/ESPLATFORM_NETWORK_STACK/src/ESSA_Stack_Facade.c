...
#define WD_KICK_PERIOD (2000)
    ...
    while (true)
    {
        ...
        {
            u8AliveCntr = 0U;
            task_mon_i_am_alive(TASK_ID_ESSA_SP_HANDLER);
            task_mon_low_stack_check(TASK_ID_ESSA_SP_HANDLER);
        }
    }
}
...
void ESSA_Stack_Facade_Init()
{
    ...
    task_mon_register_for_stats(TASK_ID_ESSA_SP_HANDLER, ESSA_SP_TaskHandle);

    task_mon_task_initialized(TASK_ID_ESSA_SP_HANDLER);
}
