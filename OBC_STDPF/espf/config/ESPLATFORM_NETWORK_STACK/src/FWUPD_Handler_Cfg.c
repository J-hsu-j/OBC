...
#define EXT_MAP_ENTRIES_CNT    (2U) /**< number of entries in the file handlers map */
...
static uint32_t process_file_footer(const char *const pFileName, sTransferInfo_t *const pTransferInfo)
{
    ...
    fres = fm_open(&fHnd, pFileName, FM_MODE_READ | FM_MODE_OPEN_EXISTING);

    if (FM_RES_OK == fres)
    {
        fres = fm_size(&fHnd, &size);
        if ((FM_RES_OK == fres) && (size >= sizeof(pTransferInfo->footer_ex)))
        {
            fres = fm_seek(&fHnd, size - sizeof(pTransferInfo->footer_ex));

            if (FM_RES_OK == fres)
            {
                uint32_t bytes_read = 0U;

                fres = fm_read(&fHnd, &pTransferInfo->footer_ex, sizeof(pTransferInfo->footer_ex), &bytes_read);

                if ((FM_RES_OK == fres) && (bytes_read == sizeof(pTransferInfo->footer_ex)))
                {
