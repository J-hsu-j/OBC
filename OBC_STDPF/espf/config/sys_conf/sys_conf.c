...
***************************************************************************************************
*/
/** @brief Seed for the CRC calculation */
...
***************************************************************************************************
*/
/** @brief Memory address from where to read the configuration (defined in the linker script) */
extern int __MODULE_CONFIG_START;

/** @brief Configuration structure read from memory */
const instance_cfg_t *const p_system_configuration = (const instance_cfg_t *const)(void *)&__MODULE_CONFIG_START;

/** @brief Configuration ASCII signature */
...
***************************************************************************************************
*/
/** @brief Verify that the configuration is compatible with the HW peripherals */
...
/** @brief Set the configuration error flag */
...
void sys_conf_init(void)
{
    ...
    const uint16_t supported_version = 0x0004;

    if (p_system_configuration->version != supported_version)
    {
        set_error_flag();
        return;
    }
...
static bool eps_compatibility_check(const safe_bool_t *p_modules)
{
    ...
    {
        if (IS_TRUE(p_modules[SYS_MODULE_ID_EPS_II]) || IS_TRUE(p_modules[SYS_MODULE_ID_EPS_III]) || IS_TRUE(p_modules[SYS_MODULE_ID_EPS_M]))
        {
    ...
    else // EPS I not active
    {
        ...
        {
            if (IS_TRUE(p_modules[SYS_MODULE_ID_EPS_M]) || IS_TRUE(p_modules[SYS_MODULE_ID_EPS_III]))
            {
        ...
        {
            if (IS_TRUE(p_modules[SYS_MODULE_ID_EPS_III]))
            {
...
static bool adcs_compatibility_check(const safe_bool_t *p_modules)
{
    ...
    {
        b_compatible = true; // ADCS configuration is consistent
    }
