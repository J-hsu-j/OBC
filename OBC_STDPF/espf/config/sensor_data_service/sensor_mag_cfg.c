...
/** @brief Magnetometer conversion constant to T for 4 gauss range */
#define LIS3MDL_4GAUSS_LSB ((double)(1.00 / 6842.00))

/** @brief Gauss to Tesla conversion constant */
#define GAUSS_TO_TESLA ((double)10000)

/** @brief Magnetometer conversion constant G -> T for 4 Gauss range */
#define LIS3MDL_4GAUSS_RANGE_LSB_TO_T ((double)LIS3MDL_4GAUSS_LSB / GAUSS_TO_TESLA)
 ...
 *
 **********************************************************************/
STATIC sds_op_status_t sensor_mag_prim_get_data_in_T(sds_mag_data_t *const p_data_dst);
STATIC sds_op_status_t sensor_mag_sec_get_data_in_T(sds_mag_data_t *const p_data_dst);
STATIC sds_op_status_t sensor_mag_conv_raw_data_to_T(uint8_t const *const p_raw_data, sds_mag_data_t *const p_data_dst);
...
sds_mag_data_if_t mag_data_prim_if = {
    .get_mag_data = sensor_mag_prim_get_data_in_T,
};

sds_mag_data_if_t mag_data_sec_if = {
    .get_mag_data = sensor_mag_sec_get_data_in_T,
};
...
STATIC sds_op_status_t sensor_mag_prim_get_data_in_T(sds_mag_data_t *const p_data_dst)
{
    ...
    return sensor_mag_conv_raw_data_to_T(&regValue[0], p_data_dst);
}

/** @brief Gets the data from the secondary magnetometer and converts it in T
 *  @param[out] sds_mag_data_t * const p_data_dst
 *  @return     sds_op_status_t */
STATIC sds_op_status_t sensor_mag_sec_get_data_in_T(sds_mag_data_t *const p_data_dst)
{
...
/** @brief Gets the data from the magnetometer and converts it in T
...
STATIC sds_op_status_t sensor_mag_conv_raw_data_to_T(uint8_t const *const p_raw_data, sds_mag_data_t *const p_data_dst)
{
    ...
    ES_TRACE_DEBUG("%s MTM RAW DATA---X:%d, Y:%d, Z:%d\r", SDS_DBG_HEAD, DataRaw[0], DataRaw[1], DataRaw[2]);

    /* Convert to T */
    mag_data_T.mag_x_T = DataRaw[0] * LIS3MDL_4GAUSS_RANGE_LSB_TO_T;
    mag_data_T.mag_y_T = DataRaw[1] * LIS3MDL_4GAUSS_RANGE_LSB_TO_T;
    mag_data_T.mag_z_T = DataRaw[2] * LIS3MDL_4GAUSS_RANGE_LSB_TO_T;

    ES_TRACE_DEBUG("%s MTM CONV DATA---X:%d, Y:%d, Z:%d\r", SDS_DBG_HEAD, mag_data_T.mag_x_T, mag_data_T.mag_y_T, mag_data_T.mag_z_T);

    /* Copy to destination buffer */
    *p_data_dst = mag_data_T;
