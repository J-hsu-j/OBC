...
STATIC aocs_cntrl_res_t aocs_cntrl_cubeadcs_int_get_cms_status(uint8_t *const p_cms_status)
{
    CRIT_ASSERT(NULL != p_cms_status);

    uint8_t read_status;

    read_status = (uint8_t)cubeadcs_get_cms_status();

    /* Make the result explicit for safety */
    switch (read_status)
    {
        case eCUBEADCS_NOT_DONE:
            *p_cms_status = CMS_NOT_DONE;
            break;

        case eCUBEADCS_COMPLETE:
            *p_cms_status = CMS_COMPLETE;
            break;
