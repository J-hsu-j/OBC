...
typedef struct
{
    ...
    eps_iii_ctrl_ch_t         channel_eps_iii; /**> Channel number of the eps iii */
...
eps_ctrl_result_t eps_ctrl_cfg_set_channel_output(const eps_ctrl_request_t *const p_request)
{
    ...
    // EPS II
    else if ((sys_conf_is_module_active(SYS_MODULE_ID_EPS_II)) && (eps_ii_set_channel_output(channel_outputs[p_request->channel].channel_eps_ii, p_request->state)))
    {
        get_lock();
    ...
    // EPS III
    else if ((sys_conf_is_module_active(SYS_MODULE_ID_EPS_III)) && (eps_iii_set_channel_output(channel_outputs[p_request->channel].channel_eps_iii, p_request->state)))
    {
        get_lock();
        // Reset the value of the last sent request (for that channel)
        ch_last_request[p_request->channel].success = false;
    ...
    // EPS M
    else if ((sys_conf_is_module_active(SYS_MODULE_ID_EPS_M)) && (eps_m_set_channel_output(channel_outputs[p_request->channel].channel_eps_m, p_request->state)))
    {
        get_lock();
        // Reset the value of the last sent request (for that channel)
        ch_last_request[p_request->channel].success = false;
...
eps_ctrl_result_t eps_ctrl_cfg_get_channel_output(const eps_ctrl_request_t *const p_request)
{
    ...
    // EPS I
    if ((sys_conf_is_module_active(SYS_MODULE_ID_EPS_I)) && (true == eps_i_get_channel_outputs()))
    {
        get_lock();
    ...
    // EPS II
    else if ((sys_conf_is_module_active(SYS_MODULE_ID_EPS_II)) && (true == eps_ii_get_channel_output(channel_outputs[p_request->channel].channel_eps_ii)))
    {
        get_lock();
        // Keep track of the request sequence ID
        active_request_seq_id = eps_ii_last_pdm_request_seq_id();
    ...
    // EPS III
    else if ((sys_conf_is_module_active(SYS_MODULE_ID_EPS_III)) && (true == eps_iii_get_channel_output(channel_outputs[p_request->channel].channel_eps_iii)))
    {
        get_lock();
        // Reset the value of the last sent request (for that channel)
        ch_last_request[p_request->channel].success = false;
    ...
    // Third Party EPS
    else if (sys_conf_is_module_active(SYS_MODULE_ID_EPS_M) && eps_m_get_channel_outputs())
    {
        get_lock();
        // Reset the value of the last sent request (for that channel)
        ch_last_request[p_request->channel].success = false;
...
void eps_ctrl_cfg_get_ch_notif_eps_iii(bool state)
{
    ...
    ch_last_request[active_request_channel].state = state;
...
{
    DATA_CACHE_EpsOverallBatteryInfoType_t eps_data;
    conops_config_t                        conops_cfg;

    Nvm_GetBlockById(NVM_APP_BLOCK_CONOPS_CONFIG, &conops_cfg);

    if (DC_DATA_STATUS_OK == dc_get_epsoverallbatteryinfo_data(&eps_data))
    {
        fdir_agent_id_t agent_id = FDIR_AGENT_INVALID;

        if (sys_conf_is_module_active(SYS_MODULE_ID_EPS_III))
        {
        ...
        else if (sys_conf_is_module_active(SYS_MODULE_ID_EPS_I))
        {
            agent_id = FDIR_AGENT_EPS_I;
        }
        else
        {
            // No known EPS configured. This is a configuration fault
            BREAK_ASSERT(false);
        }

        if ((FDIR_AGENT_INVALID != agent_id) && (eps_data.u16Voltage < conops_cfg.thresh.i32V_batt_safe))
        {
            fdir_srv_log_if_clear(FDIR_FAULT_EPS_BATT_VOLTAGE_BELOW_SAFE, agent_id, "Battery voltage (%d mV) is below safe level (%d mV)", eps_data.u16Voltage, conops_cfg.thresh.i32V_batt_safe);

            fdir_srv_fault_set_state(FDIR_FAULT_EPS_BATT_VOLTAGE_BELOW_SAFE, agent_id, true, NULL);
        }
    }
}
