...
void dc_cfg_init(void)
{
    ...
    {
        task_mon_register_for_stats(TASK_ID_DATACACHE, task_hnd);
        task_mon_task_initialized(TASK_ID_DATACACHE);
    }
}
...
static void dc_cfg_task(void *fp_pvArg)
{
    while (1U)
    {
        ...
        task_mon_i_am_alive(TASK_ID_DATACACHE);
        task_mon_low_stack_check(TASK_ID_DATACACHE);
    }
