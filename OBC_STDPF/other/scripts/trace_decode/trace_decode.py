...
# timestamp regexp pattern string:
# expected format of the timestamp is the following:
# <unix timestamp>.<ms part><us part>
ts_pattern : str = r'^(\d{1,10})(?:\.(\d{0,6}))?$'
regex = None
...
def convert_log_to_printable_data(raw_log_data: bytearray) -> None | str:
    ...
    try:
        ...
        if received_crc == calculated_crc:
            ts_match = regex.match(split[3])

            unix_timestamp = ts_match.group(1) if ts_match.group(1) != None else 0
            sub_second_timestamp = ts_match.group(2) if ts_match.group(2) != None else 0

            # convert from UNIX time stamp
            tstamp = str(datetime.datetime.utcfromtimestamp(int(unix_timestamp)))
            split[3] = f'{tstamp}.{sub_second_timestamp:06}'
