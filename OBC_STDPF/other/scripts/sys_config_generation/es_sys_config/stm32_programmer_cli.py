...
def obc_connect(log: bool = False, verbose: bool = False) -> bool:
    ...
    cmd_return = subprocess.run(cmd, capture_output=True, shell=True, text=True, check=False, encoding="unicode_escape")
...
def is_stm32_cli_installed(verbose=False) -> bool:
    ...
    if verbose:
        typer.secho(f'Calling {cmd}')
    ret = subprocess.run(cmd, capture_output=True, shell=True, text=True, check=False, encoding="unicode_escape")
    if verbose:
